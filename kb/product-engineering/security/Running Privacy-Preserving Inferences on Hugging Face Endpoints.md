...
secondary_topics:
- inference/serving
summary: Shows how to serve Zama Concrete ML models under Fully Homomorphic Encryption
  on HF Inference Endpoints via custom inference handlers, so a spam classifier runs
  on ciphertext without ever seeing the plaintext message; also covers compiling your
  own FHE-friendly model.
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:06:28Z'
classifier: claude
taxonomy_rev: 1
words: 1807
content_sha256: f58fc30b0365f8a80aad96b6eecbfb99488449b263bfa3560af3f9e5aa9498f1
---
...
#### zama-fhe/concrete-ml-encrypted-decisiontree

![](https://cdn-avatars.huggingface.co/v1/production/uploads/1658839785577-62dad700f8148e3d4b514343.jpeg) 
...
Let's start with deploying an FHE-friendly model (prepared by Zama or third parties - see [Preparing your pre-compiled model](https://huggingface.co#preparing-your-pre-compiled-model) section below for learning how to prepare yours).
...
Like with any other model available on the Hugging Face platform, select *Deploy* and then *Inference Endpoint (dedicated)*:
 ...
 ![Inference Endpoint (dedicated)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/fhe-endpoints/inference_endpoint.png) 

*Inference Endpoint (dedicated)*

Next, choose the Endpoint name or the region, and most importantly, the CPU (Concrete ML models do not use GPUs for now; we are [working](https://www.zama.ai/post/tfhe-rs-v0-5) on it) as well as the best machine available - in the example below we chose eight vCPU. Now click on *Create Endpoint* and wait for the initialization to finish.

 ![Create Endpoint](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/fhe-endpoints/create_endpoint.png) 

*Create Endpoint*

After a few seconds, the Endpoint is deployed, and your privacy-preserving model is ready to operate.

 ![Endpoint is created](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/fhe-endpoints/endpoint_is_created.png) 
...
The goal is not only to deploy your Endpoint but also to let your users play with it. For that, they need to clone the repository on their computer. This is done by selecting *Clone Repository*, in the dropdown menu:
 ...
 ![Clone Repository](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/fhe-endpoints/clone_repository.png) 
...
```
Of course, fill it in with with *your* Entrypoint’s URL. Also, define an [access token](https://huggingface.co/docs/hub/en/security-tokens) and store it in an environment variable:
...
guideprivacyresearch
 ...
 August 2, 2023 guideprivacyresearch
