---
title: Mastering remote engineering work from your phone | OpenAI Developers
topic: product-engineering
subtopic: ux-patterns
secondary_topics: []
summary: 'Power-user field guide to Codex Remote in the ChatGPT mobile app, framing
  the phone as a control plane for agents running on your own machines: worktrees,
  goals, side chats, inline code review, queued vs. steering prompts, and command-approval
  security controls.'
source: openai-devs
url: https://developers.openai.com/blog/mastering-codex-remote-for-engineering/
author: null
published: '2026-06-23'
fetched: '2026-07-11T04:12:02Z'
classifier: claude
taxonomy_rev: 1
words: 2343
content_sha256: f1e87f9c701f004bc6777aae3ffecbe3575d17265f3811e03e55b6e13fc562db
---
...
Over the last two months, we have added a surprising amount of depth to that loop: remote host connections, worktrees, goals, side chats, inline code review, queued and steering prompts, attachments, skills and plugins, archived tasks, security controls, and a long tail of small details that make the app useful for serious work.

This is the field guide I wish every new power user had.

![Remote project picker in the ChatGPT mobile app](https://developers.openai.com/images/blog/mastering-codex-remote-for-engineering.jpg)
...
- Should this become a durable goal, a separate task, or a quick side question?
...
Good agent work starts with a correctly scoped environment. Remote lets you choose the connected host and workspace before the first prompt. For a new task, you can also choose a branch, create a separate worktree, and run the environment setup associated with it.

![Remote controls for choosing a host, repository, worktree, and environment](https://developers.openai.com/images/blog/mastering-codex-remote-worktree-branch.jpg)
...
![Remote composer with multiple skills and plugins added inline](https://developers.openai.com/images/blog/mastering-codex-remote-skills-plugins.jpg)
...
Long-running coding tasks accumulate valuable context. Interrupting one with every side question makes the main transcript noisy and can pull the agent away from the objective.

That is what side chats solve.

Use `/side` to open a lightweight conversation connected to the current task. Use `/side <prompt>` to open it with a question already prepared. Even better, select text in the transcript and choose **Ask in side chat**. The selected passage becomes the starting context for the new conversation.

![Selected transcript text with the Ask in side chat action](https://developers.openai.com/images/blog/mastering-codex-remote-side-chat.jpg)
...
The distinction is useful: The main task owns the work; the side chat helps me understand the work.
...
![Remote diff with an inline review comment](https://developers.openai.com/images/blog/mastering-codex-remote-inline-review.jpg)
...
- Link a file back into the task when you want Codex to reason about that file specifically.

This makes a tight mobile review loop possible:

- Codex completes the implementation.
- I inspect the diff from my phone.
- I leave two inline comments.
- Codex addresses them in the same task.
- I review the smaller follow-up diff.
...
Remote surfaces approval requests for commands, file changes, network access, and connected tools. Depending on the request and host configuration, an approval may apply once, for the current task, or more broadly.

The power-user move is not to approve everything. It is to choose the narrowest permission that keeps the work moving.

For a well-understood command in a trusted task, a task-scoped approval can remove repetitive interruptions. For an unfamiliar command, a sensitive repository, or a request whose effect is unclear, approve once—or deny it and ask Codex to explain or use a safer approach.

You can also choose the task’s broader approval behavior when starting work. Think of this as part of task setup, alongside the host, workspace, branch, and model.

## 7. Manage context before it becomes a problem

Agent tasks are stateful, and long tasks eventually accumulate enough context to become slower or less focused.
...
- `/compact`compresses an overgrown task while preserving the useful working state.
- `/fork`creates a new task from the current one when you want shared history but a different direction.
...
Do not use a side chat and a fork interchangeably. A side chat is a lightweight question around the current work. A fork is a new primary task that inherits the original task’s history.

## 8. Keep a clean Task Desk

My own Codex workflow increasingly looks like a small operations desk.

I pin the few tasks that are actively moving, rename them around outcomes, and archive them aggressively once the work is done. Browsing archived tasks makes this safe: Archive is organization, not deletion.
...
Spotlight and Shortcuts can open Remote directly. On iPad, keyboard shortcuts make the app surprisingly efficient: Create a new task, move between tasks, open changed files, pin, rename, and archive without reaching for every control.
...
| `/side [question]` | Ask a side question without disrupting the main task. |
| `/review` | Review local changes or compare them with a branch. |
| `/status` | Inspect the session, workspace, context, and rate limits. |
| `/compact` | Compress a long task’s context. |
| `/fork` | Create a new primary task from the current history. |
...
Start a focused task for a release or pull request. Ask Codex to check the current branch, CI state, open review feedback, and release inclusion. Pin the task. When new information arrives, steer only when it invalidates the active investigation; otherwise, queue it. Review the final diff or release note from the phone and archive the task when the release lands.

### The interrupt-driven bug fix

Attach a screenshot, log, or captured file. Ask Codex to diagnose before editing. Use a side chat to interrogate one suspicious error without derailing the main investigation. Once the cause is clear, return to the main task and authorize the narrow fix.
...
- Browse archived tasks instead of keeping the active list crowded.
