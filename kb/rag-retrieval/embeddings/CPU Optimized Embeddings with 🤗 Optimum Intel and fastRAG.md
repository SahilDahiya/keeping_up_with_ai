...
summary: Speeds up bge-base embeddings on Xeon CPUs by quantizing to int8 with Optimum
  Intel / IPEX, reporting latency and MTEB retrieval-quality deltas, then wires the
  optimized encoder into a fastRAG retrieval pipeline.
triage: null
skip_reason: null
source: huggingface
url: https://huggingface.co/blog/intel-fast-embedding
author: Peter Izsak; Moshe Berchansky; Daniel Fleischer; Ella Charlaix; Morgan Funtowicz;
  Moshe Wasserblat
published: '2024-03-15'
fetched: '2026-07-14T22:10:37Z'
classifier: claude
taxonomy_rev: 1
words: 2000
content_sha256: f3d1342c076e0aa402d8c783b2281de71d5767e87fc10a57604afc552234239a
---
...
- Using [Intel extension for PyTorch](https://intel.github.io/intel-extension-for-pytorch/#introduction)(IPEX) runtime with`bf16`and tracing the model using torchscript.
...
We can see that the quantized model has the best latency overall, under 10 ms for the small and base models and <20 ms for the large model. Compared to the original model, the quantized model shows up to 4.5x speedup in latency.
 ...
 ![latency](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/178_intel_ipex_quantization/latency.png)
...
Results show that the quantized models reach higher throughput values compared to the other models, and reach peak throughput at batch size 128. Overall, for all model sizes, the quantized model shows up to 4x improvement compared to the baseline `bf16` model in various batch sizes.
 ...
 ![throughput small](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/178_intel_ipex_quantization/throughput_small.png)


*Figure 2. Throughput for BGE small.*

 ![throughput base](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/178_intel_ipex_quantization/throughput_base.png)


*Figure 3. Throughput for BGE base.*

 ![throughput large](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/178_intel_ipex_quantization/throughput_large.png)


*Figure 4. Throughput for BGE large.*

 
![](https://github.com/IntelLabs/fastRAG/raw/main/assets/fastrag_header.png) 
...
For more RAG-related methods, models and examples we invite the readers to explore [fastRAG/examples](https://github.com/IntelLabs/fastRAG/tree/main/examples) notebooks.
 ...
 Feature Extraction •  33.4M • Updated   •  66.9M  •  509 

 Feature Extraction •  0.6B • Updated   •  12.9M  •  1.22k 

 Sentence Similarity •  33.4M • Updated   •  427k  •  189 

 Viewer • Updated  •  1.59k •  5.87k  •  103 
