...
- models/releases
- infra-platform/edge
summary: 'EmbeddingGemma is a 308M-param multilingual embedding model: a Gemma3 backbone
  converted to bidirectional attention plus mean pooling and two dense layers, trained
  on ~320B tokens with Matryoshka Representation Learning so its 768-dim output can
  be truncated to 512/256/128; runs under 200 MB RAM quantized, tops MTEB under 500M,
  and the post shows a domain fine-tune on MIRIAD that beats models twice its size.'
triage: null
skip_reason: null
source: huggingface
url: https://huggingface.co/blog/embeddinggemma
author: Tom Aarsen; Joshua; Alvaro Bartolome; Aritra Roy Gosthipaty; Pedro Cuenca;
  Sergio Paniego
published: '2025-09-04'
fetched: '2026-07-14T22:05:16Z'
classifier: claude
taxonomy_rev: 1
words: 6573
content_sha256: e021cf7c45d18a5615699efeb18bc54786b3338f09769c7cb748d079e5f2800a
---
...
- specify prompt names (e.g. "Reranking"),
- specify prompt strings (e.g. "task: search result | query: "), or
- manually prepend the prompts to your input text.
...
# tensor([1, 2, 3, 0])
```
- [Sentence Transformers](https://sbert.net/docs/package_reference/sentence_transformer/SentenceTransformer.html#sentence_transformers.SentenceTransformer.encode_query)- `encode_query`method documentation
- [Sentence Transformers](https://sbert.net/docs/package_reference/sentence_transformer/SentenceTransformer.html#sentence_transformers.SentenceTransformer.encode_document)- `encode_document`method documentation
- [Sentence Transformers](https://sbert.net/docs/package_reference/sentence_transformer/SentenceTransformer.html#sentence_transformers.SentenceTransformer.similarity)- `similarity`method documentation
...
```
If you run the Docker container with the

`cuda-1.8.1`tag, it includes support for multiple GPU architectures: Turing, Ampere, Ada Lovelace, and Hopper. For a lighter image tailored to just your GPU, you can instead use a specific tag such as`turing-1.8.1`,`1.8.1`and`86-1.8.1`(Ampere),`89-1.8.1`(Ada Lovelace), or`hopper-1.8.1`.
...
|---|---|---|
| `BAAI/bge-base-en-v1.5` | 109M | 0.7541 | 
| `intfloat/multilingual-e5-small` | 118M | 0.6852 | 
| `ibm-granite/granite-embedding-125m-english` | 125M | 0.7745 | 
| `Snowflake/snowflake-arctic-embed-m-long` | 137M | 0.7514 | 
| `intfloat/multilingual-e5-base` | 278M | 0.7052 | 
| `Snowflake/snowflake-arctic-embed-m-v2.0` | 305M | 0.8467 | 
| `BAAI/bge-large-en-v1.5` | 335M | 0.7727 | 
| `mixedbread-ai/mxbai-embed-large-v1` | 335M | 0.7851 | 
| `intfloat/multilingual-e5-large` | 560M | 0.7318 | 
| `Snowflake/snowflake-arctic-embed-l-v2.0` | 568M | 0.8433 | 
| `Qwen/Qwen3-Embedding-0.6B` | 596M | 0.8493 | 
| (base)`google/embeddinggemma-300m` | 268M | 0.8340 | 
| (fine-tuned)`sentence-transformers/embeddinggemma-300m-medical` | 268M | 0.8862 | 
...
- [Binary and Scalar Embedding Quantization for Significantly Faster & Cheaper Retrieval](https://huggingface.co/blog/embedding-quantization): shrink EmbeddingGemma embeddings further at retrieval time.
 ...
 Sentence Similarity •  0.1B • Updated   •  33.4k  •  38 

 Sentence Similarity •  0.3B • Updated   •  198k  •  106 

 Sentence Similarity •  0.3B • Updated   •  1.54M  •  1.79k 

 Sentence Similarity •  0.1B • Updated   •  39.1k  •  38 

 Sentence Similarity •  0.3B • Updated   •  7.19M  •  373 

 Sentence Similarity •  0.1B • Updated   •  15M  •  360 

 Feature Extraction •  0.3B • Updated   •  4.88M  •  814 

 Sentence Similarity •  Updated   •  8.11k  •  50 

 Viewer • Updated  •  4.49M •  1.31k  •  36 

 Viewer • Updated  •  8.98M •  289  •  3 
...
nlpguidecommunity
 ...
 July 1, 2025 nlpguidecommunity

  195

 March 26, 2025 Great post well done 👏🏻 I have a quick question. Can you please share the package versions for training? For example I needed to update my torch version to torch>=2.6.
...
```
Until the next transformers version is out, then you can start using that. Edit: That's version `transformers>=4.56.2`.
...
![image.png](https://cdn-uploads.huggingface.co/production/uploads/61b253b7ac5ecaae3d1efe0c/rbdK92vArqbO5dX7TFvcj.png)


Is there any documentation to use this model in android device?

It depends on how you intend to use it. If you're creating a knowledge base similar to a RAG, you download the model and use it offline. But you also need to create the vectorization process.

•

 The sagemaker deployment ([https://huggingface.co/google/embeddinggemma-300m?sagemaker_deploy=true](https://huggingface.co/google/embeddinggemma-300m?sagemaker_deploy=true)) is not working because it uses hugging-face tei version 1.8.0 (while it should be >=1.8.1 as the article mentions):
...
Hey [@etiennekintzler](https://huggingface.co/etiennekintzler)  thanks for flagging! Indeed we're working close with the AWS SageMaker team on releasing those, we'll ping you once released but should be soon 🤗 
