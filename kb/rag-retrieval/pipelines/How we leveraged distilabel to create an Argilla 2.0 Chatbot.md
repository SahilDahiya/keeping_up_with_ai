...
secondary_topics:
- models/fine-tuning
summary: 'End-to-end build of a docs chatbot: distilabel generates synthetic query/answer
  pairs from Argilla 2.0 documentation, which fine-tunes a bge-base Matryoshka embedding
  model used in a retrieval + Gradio chat pipeline.'
triage: null
skip_reason: null
source: huggingface
url: https://huggingface.co/blog/argilla-chatbot
author: Agustín Piqueres Lajarín; Gabriel Martín Blázquez; Sara Han Díaz; Omar Sanseviero;
  Daniel Vila
published: '2024-07-16'
fetched: '2026-07-14T22:06:12Z'
classifier: claude
taxonomy_rev: 1
words: 7270
content_sha256: c6f4ec959a32d1327716d6eb7762794c7abb42692ac319ba5abdc45dfae02bff
---

# How we leveraged distilabel to create an Argilla 2.0 Chatbot

Sentence Similarity •  0.1B • Updated   •  19  •  5  

#### plaguss/bge-base-argilla-sdk-matryoshka

![](https://cdn-avatars.huggingface.co/v1/production/uploads/6435d564a4bd75c62cc03701/7P2G_wVNB6MISp2Phh427.jpeg) 
...
![argilla-sdk-chatbot](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/argilla-chatbot/chatbot.png)


- [Generating Synthetic Data for Fine-Tuning a domain-specific Embedding Models](https://huggingface.co#generating-synthetic-data-for-fine-tuning-domain-specific-embedding-models)
- [Downloading and chunking data](https://huggingface.co#downloading-and-chunking-data)
- [Generating synthetic data for our embedding model using distilabel](https://huggingface.co#generating-synthetic-data-for-our-embedding-model-using-distilabel)
- [Explore the datasets in Argilla](https://huggingface.co#explore-the-datasets-in-argilla)
- [Fine-Tune the embedding model](https://huggingface.co#fine-tune-the-embedding-model)
- [The vector database](https://huggingface.co#the-vector-database)
- [Connect to the database](https://huggingface.co#connect-to-the-database)
- [Create the table with the documentation chunks](https://huggingface.co#create-the-table-with-the-documentation-chunks)
- [Creating our ChatBot](https://huggingface.co#creating-our-chatbot)
- [The Gradio App](https://huggingface.co#the-gradio-app)
- [Deploy the ChatBot app on Hugging Face Spaces](https://huggingface.co#deploy-the-chatbot-app-on-hugging-face-spaces)
- [Playing around with our ChatBot](https://huggingface.co#playing-around-with-our-chatbot)
...
Lastly, we use [ ExpandColumns](https://distilabel.argilla.io/dev/components-gallery/steps/expandcolumns/) to move the previous column of positive to different lines. As a result, each 

`positive` query will occupy a separate line, while the `anchor` and `negative` columns will be replicated to match the expanded positive queries. This data manipulation will yield a dataset with the ideal structure for fine-tuning:```
...
dataset.create()
```
Now, let's retrieve the dataset we created earlier from the Hugging Face Hub. Recall the dataset we generated in the [chunking data section](https://huggingface.co#downloading-and-chunking-data)? We'll download that dataset and extract the essential columns we need to move forward:
...
Now, we can repeat the process with the dataset ready for fine-tuning we generated in the [previous section](https://huggingface.co#generating-synthetic-data-for%E2%80%93our-embedding-model:-distilabel-to-the-rescue). 
...
```
Let's take a closer look at the structure of our dataset, which consists of three essential [ TextFields](https://argilla-io.github.io/argilla/latest/reference/argilla/settings/fields/?h=textfield): 

`anchor`, `positive`, and `negative`. The `anchor` field represents the chunk of text itself, while the `positive` field contains a query that can be answered using the anchor text as a reference. In contrast, the `negative` field holds an unrelated query that serves as a negative example in the triplet. The positive and negative questions play a crucial role in helping our model distinguish between these examples and learn effective embeddings.An example can be seen in the following image:
...
To guide us through this step, we'll be referencing the [ train_embedding.ipynb](https://github.com/argilla-io/argilla-sdk-chatbot/blob/main/train_embedding.ipynb) notebook, which draws inspiration from Philipp Schmid's 
...
Fortunately, working with inference endpoints is a breeze, thanks to the [ inference client](https://huggingface.co/docs/text-generation-inference/basic_tutorials/consuming_tgi#inference-client) from the 
...
```
This function will take two arguments: `message` and `history` courtesy of the gradio [ ChatInterface](https://www.gradio.app/docs/gradio/chatinterface), obtain the documentation pieces from the database to help the LLM with the response, and prepare the prompt to be passed to our 

`LLM` model.These are the `system_prompt` and the prompt template used. They are heavily inspired by [ wandbot](https://github.com/wandb/wandbot) from Weights and Biases.
...
[https://huggingface.co/spaces/plaguss/argilla-sdk-chatbot-space](https://huggingface.co/spaces/plaguss/argilla-sdk-chatbot-space)
