...
secondary_topics:
- models/fine-tuning
summary: 'Releases six CrossEncoder rerankers (17M-1B) built on Ettin ModernBERT encoders
  with the full training recipe: data selection, loss choice, hard-negative mining,
  and BEIR/NanoBEIR numbers showing SOTA at each size.'
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:08:43Z'
classifier: claude
taxonomy_rev: 1
words: 6386
content_sha256: 25ec7e68e86afcfcc65c666ec045a1a6b8fe2d537cfb27c482139143a891c1fa
---

# Introducing the Ettin Reranker Family

Text Ranking •  0.1B • Updated   •  6.74k  •  2  

#### cross-encoder/ettin-reranker-150m-v1

![](https://cdn-avatars.huggingface.co/v1/production/uploads/1609621301279-5eff4688ff69163f6f59e66c.png) 
...
`cross-encoder/ettin-reranker-v1-data``lightonai/embeddings-pre-training``lightonai/embeddings-fine-tuning`![Our six rerankers paired with embeddinggemma-300m on MTEB(eng, v2) Retrieval](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/ettin-reranker/mteb_ndcg10_embeddinggemma-300m.png)


*Our six rerankers paired with *

`google/embeddinggemma-300m` on MTEB(eng, v2) Retrieval. See Results for five more embedder pairings.If you're new to rerankers and want the "why" first, jump to [What is a reranker, and why pair one with an embedder?](https://huggingface.co#what-is-a-reranker-and-why-pair-one-with-an-embedder). If you just want to plug a model in, jump to [Usage](https://huggingface.co#usage). If you want to train your own, jump to [Training](https://huggingface.co#training).

I bootstrapped the training recipe below with the new

[shipped in](https://github.com/huggingface/sentence-transformers/tree/main/skills)`train-sentence-transformers`Agent Skill[Sentence Transformers v5.5.0](https://github.com/huggingface/sentence-transformers/releases/tag/v5.5.0). Install it with`hf skills add train-sentence-transformers [--global] [--claude]`and ask your AI coding agent (Claude Code, Codex, Cursor, Gemini CLI, ...) to fine-tune a`SentenceTransformer`,`CrossEncoder`, or`SparseEncoder`model on your data.

- [What is a reranker, and why pair one with an embedder?](https://huggingface.co#what-is-a-reranker-and-why-pair-one-with-an-embedder)
- [Usage](https://huggingface.co#usage)
- [Architecture Details](https://huggingface.co#architecture-details)
- [Results](https://huggingface.co#results)
- [Training](https://huggingface.co#training)
...
`model_kwargs={"dtype": "bfloat16", "attn_implementation": "flash_attention_2"}` for the highest throughput. See the ```
from sentence_transformers import CrossEncoder
model = CrossEncoder(
    "cross-encoder/ettin-reranker-32m-v1",
...
The overall strongest reranker in the comparison is `Qwen/Qwen3-Reranker-4B` at 0.6367 MTEB, +0.025 above our 1B model. Closing that gap from the current recipe would likely require distilling from a stronger teacher (our teacher itself sits below `Qwen3-Reranker-4B`). For most retrieve-then-rerank workloads, our 1B at a quarter of the parameters (see [Speed](https://huggingface.co#speed)) is a much more practical pick.
...
`max_length=512` to avoid giving the older models an unfair advantage. Each model uses its best supported attention implementation: Flash Attention 2 wherever the architecture supports it (BERT, XLM-RoBERTa, ModernBERT, Qwen2), SDPA where it doesn't, and eager for DeBERTa-v2 (which currently has neither FA2 nor SDPA support in `transformers`).For every model an auto-batch search starts at batch size 8 and doubles until the GPU runs out of memory. At each batch size I run three timed passes and keep the median throughput, so a single unlucky run doesn't drag the number around. The reported throughput is at whichever batch size won.
...
`ibm-granite/granite-embedding-reranker-english-r2`All three 150M models use Flash Attention 2, but the two peers load through `AutoModelForSequenceClassification`, which keeps the inputs padded. So attention itself runs the FA2 kernel, but the rest of the model is still doing dense compute on padding tokens that don't contribute anything. Our modular `Transformer` module (see [Architecture Details](https://huggingface.co#architecture-details) above) propagates unpadded inputs all the way through the model, so every layer only spends compute on real tokens. That's the difference between getting some of FA2's benefit and getting all of it.
...
So my recommendation is simple: enable `bf16` and FA2 together. The six Ettin rerankers will use unpadded inputs by default, since that's what the modular `Transformer` module from the [Architecture Details](https://huggingface.co#architecture-details) section is set up for. The full snippet is the same as in the [Usage](https://huggingface.co#usage) section above:
...
`pip install kernels`to install FA2. It ships pre-built kernels for a wide range of GPU architectures, CUDA versions, and operating systems.
...
`SentenceTransformer`, `CrossEncoder`, or `SparseEncoder` model on your data. The skill carries version-aware guidance for base model selection, loss and evaluator choice, hard-negative mining, distillation, LoRA, Matryoshka, multilingual training, and static embeddings, plus template scripts for each model type.```
...
- LightOn pre-training data (`lightonai/embeddings-pre-training``(query, document, similarity)`triples in total.
- Rescored retrieval data from `lightonai/embeddings-fine-tuning``msmarco`,`hotpotqa`,`trivia`,`nq`,`squadv2`,`fiqa`,`fever`). The source dataset has up to 2048 candidate documents per query (initially scored with`Alibaba-NLP/gte-modernbert-base``mixedbread-ai/mxbai-rerank-large-v2``cross-encoder/lightonai-embeddings-fine-tuning-reranked-v1`[Jang et al.](https://arxiv.org/abs/2604.04734)quantile-anchor recipe (all positives + top-16 hard + ~239 quantile-anchor stratified). For training, I pick 64 of those 256 per query: 32 from the score-sorted head (the positive plus the hardest negatives) and 32 medium-difficulty negatives sampled from a band further down the teacher's ranking. See the[dataset card](https://huggingface.co/datasets/cross-encoder/ettin-reranker-v1-data)for the exact rank positions.
...
- **Models**:
- **Dataset**:- `cross-encoder/ettin-reranker-v1-data`- `(query, document, label)`triples, kept as 39 named splits so the provenance of every row is visible.
- **Training script**: the ~150 lines in- [Overall Training Script](https://huggingface.co#overall-training-script)above, which is the same script used for all six models.
...
```
 ...
 Text Ranking •  16.8M • Updated   •  7.19k  •  11 

 Text Ranking •  1B • Updated   •  15.4k  •  6 

 Text Ranking •  31.9M • Updated   •  9.86k  •  3 

 Text Ranking •  0.4B • Updated   •  5.16k  •  3 

 Text Ranking •  68.1M • Updated   •  9.88k  •  3 

 Sentence Similarity •  0.3B • Updated   •  1.54M  •  1.79k 

 Fill-Mask •  Updated   •  3.34k  •  19 

 Feature Extraction •  Updated   •  1.32k  •  13 

 Text Ranking •  2B • Updated   •  51k  •  142 

 Viewer • Updated  •  143M •  722  •  8 

 Preview • Updated   •  279  •  8 

 Viewer • Updated  •  53.7M •  2.46k  •  21 

 Viewer • Updated  •  1.38B •  2.44k  •  48 

 Viewer • Updated  •  100k •  3.42k  •  49 
...
- +5
 ...
 May 24, 2024 llmmoelong-context

  50

 April 24, 2026 This is a fantastic read, thank you for the hard work and for sharing the recipes!
...
Great work! Love that you used stratified sampling.

It's great to see its power on cross-encoders, too!
...
What a clean release, congrats [@tomaarsen](https://huggingface.co/tomaarsen) !

Thanks Maxime! You know, I bet the training script would apply pretty cleanly on LFM models as well 👀

Although maybe the model prefers a generative architecture with a chat template that asks the model to generate either "yes" or "no", and the difference in raw logit scores for those tokens are used as the prediction. That's what's used for the teacher model's Sentence Transformers integration, and it works quite well: [https://huggingface.co/mixedbread-ai/mxbai-rerank-large-v2](https://huggingface.co/mixedbread-ai/mxbai-rerank-large-v2)
...
Hello [@tomaarsen](https://huggingface.co/tomaarsen)  ,

Since I often use TEI (text-embeddings-inference) in production, I opened a pull request adding Ettin support to TEI, though I’m not sure if it will be merged.

With this, Ettin can be served as a stable API service through TEI, and I observed around a 1.5x inference speedup in my tests!

Amazing work [@tomaarsen](https://huggingface.co/tomaarsen) , thanks for everything you do for open source !

May I ask some questions on the recipe ?

- If I understand well, you are mixing lightOn hard negatives data (with Jang et al. stratified sampling) with broader lightonai/embeddings-pre-training - which (i think) doesn't include mined negatives- Did you try several mixes of pretraining / resampled hard negatives ?
...
Correct on all counts. They're different data formats initially. I took:

- query-answer pair style data from [https://huggingface.co/datasets/lightonai/embeddings-pre-training](https://huggingface.co/datasets/lightonai/embeddings-pre-training)together with
- [https://huggingface.co/datasets/lightonai/embeddings-fine-tuning](https://huggingface.co/datasets/lightonai/embeddings-fine-tuning)+ rescoring with mxbai-rerank-large-v2 + Jang et al. stratified sampling + subsampled to 64 docs per query + reformatted to 64 query-answer pairs.

Yes, I did try various mixes. Specifically, I tried multiple approaches for the second subsampling (256 -> 64), e.g. grabbing 16 random docs, grabbing top 8 + 8 random, top 16 + 16 random, top 16 + 16 uniformly sampled, top 32, top 64, and what worked best for me: 32 top + 32 uniformly subsampled.

I also tried different mixes of the pre-training data vs the embeddings-fine-tuning data. As you can see [https://huggingface.co/datasets/cross-encoder/ettin-reranker-v1-data#lighton-pre-training-32-configs](https://huggingface.co/datasets/cross-encoder/ettin-reranker-v1-data#lighton-pre-training-32-configs), I limited the largest sources from embeddings-pre-training to 5M and smaller ones to 2.5M, but during my tests I also tried limiting the largest to 1M and the smallest to 500k. Expanding to the larger number of rows was beneficial. I might also benefit from going larger, but that was exceeding my training time goals. I only experimented with the number of embeddings-fine-tuning data by updating its sampling (e.g. the top 64 has 2x as much fine-tuning data as the top 32 test).

I ended up not wanting to do too many experiments on the data mixture, as I was wary that I might start to overfit on my evaluation benchmark (NanoBEIR), which would only result in better NanoBEIR scores but worse overall MTEB scores. So I didn't do too much.

In ST, you advise to discard Arguana and Touché when using Nanobeir13, I guess it works best with those at the end ?


In truth, I probably should not have excluded these 2 from the default CrossEncoderNanoBEIREvaluator, as people might run the dense and reranker evaluators side by side and expect to be able to compare them, which you now sadly can't do. But in short: At the time, my opinion was that Arguana and Touche2020 didn't really match the usual usage for rerankers. For example, in ArguAna the query is just a big paragraph, and in Touche the documents are a bit vague (i.e. not as confident information sources but instead opinions). I think these might make them less suited as benchmarking datasets for reranker models, and I suppose also less interesting as training datasets for training rerankers. In the blogpost, I included them, as I want the scores to be easier to compare to the dense NanoBEIR13 scores from a standard embedding model.
