...
summary: Explains FireAttention V2 and the serving optimizations that make long-context
  inference more practical.
source: fireworks
url: https://fireworks.ai/blog/fireattention-v2-long-context-inference
author: null
published: '2024-06-20'
fetched: '2026-07-11T04:13:52Z'
classifier: codex
taxonomy_rev: 1
words: 819
content_sha256: d718b51f9f15a5fc3e4512fac9555d545e9c7343602d9ef47a170206ae3ad545
triage: keep
skip_reason: null
---
...
[Long contexts have many practical applications](https://x.com/dzhulgakov/status/1755339339486421037), especially in RAG, code completion, classification, etc. Yet, long context has been challenging, especially in online applications, both from [model quality](https://github.com/llmonpy/needle-in-a-needlestack) and [inference speed](https://www.tensorops.ai/post/rag-vs-large-context-models-how-gemini-1-5-changes-the-world) standpoints.
...
This post complements our [previous post](https://fireworks.ai/blog/fire-attention-serving-open-source-models-4x-faster-than-vllm-by-quantizing-with-no-tradeoffs) in the FireAttention series.
...
- •**Multi-host deployment mode**, which is beneficial for use cases with high traffic and balanced prompt/generation ratios. Majority of use cases running on[fireworks.ai](https://fireworks.ai/)fall into this category.

Many new LLMs claim long context support, yet the only widely-accepted benchmark is a ‘[needle in a haystack](https://github.com/gkamradt/LLMTest_NeedleInAHaystack)’ .We argue that this benchmark is quite a low bar for an LLM as it doesn’t require any reasoning capabilities. Chunking large contexts and using embedding-based retrieval to reduce context length would be a much more performant option in this case.

Thus we used a quite comprehensive [RULER](https://github.com/hsiehjackson/RULER) benchmarking library to test more ‘LLM-worthy’ applications. We’ve chosen the following benchmarks: multi-needle, variable tracking and question answering (squad dataset). We think that these benchmarks are the most representative of practical workloads.
...
We use [Qwen2 72B Instruct](https://huggingface.co/Qwen/Qwen2-7B-Instruct) model to measure long context performance of 32K context in fp16 mode. It is the best model by far for contexts over 8K.

For fp8 mode we use [Mixtral 8x7B ](https://fireworks.ai/blog/mixtral-8x7b-on-fireworks-faster-cheaper-even-before-the-official-release)Instruct model, which also has 32K context support.
...
Measurements were done using our [open source benchmarking library](https://github.com/fw-ai/benchmark) and thus can be easily reproduced.
...
[Qwen 72B](https://app.fireworks.ai/models/fireworks/qwen2p5-72b) on vLLM 0.5.0 vs Fireworks AI in fp16 on 8 H100 GPUs
...
[Qwen 2 72b Instruct](https://fireworks.ai/models/fireworks/qwen2-72b-instruct) , [Mixtral 8x7B Instruct](https://fireworks.ai/models/fireworks/mixtral-8x7b-instruct) as well as numerous other models are available for you to try right now on [fireworks.ai](https://fireworks.ai/) and are capable of producing speeds measured in this blog post. It’s also advised to use [dedicated deployments](https://fireworks.ai/blog/custom-models-h100s-on-demand-deployments) to ensure proper performance measurements.

Multi-host mode is only available for enterprise customers. [Contact us](https://fireworks.ai/company/contact-us) if your use case fits the long-generation pattern.
