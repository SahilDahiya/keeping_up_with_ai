...
subtopic: kernels
secondary_topics: []
summary: Reverse-engineering writeup for FlashAttention-4, explaining how kernel design
  choices affect attention performance.
source: modal
url: https://modal.com/blog/reverse-engineer-flash-attention-4
author: null
published: '2025-09-26'
fetched: '2026-07-11T04:28:08Z'
classifier: codex
taxonomy_rev: 1
words: 4138
content_sha256: 3efc88634045f34709020c1ae5caaa1003543260f33d204699f57c2ab1d8b6b0
triage: keep
skip_reason: null
---
...
*This blog post made the front page of HackerNews! Discussion  here.*

*This blog post was presented to the  GPU MODE Discord! Watch the recording here.*
...
- A [Load warp](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L837)to load query, key, and value tiles from global memory into shared memory
...
- One or two [Epilogue warps](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L1737)to store completed output tiles from shared memory into global memory

In the above discussion, we implied that each CTA works on just two query tiles and produces just two output tiles. That’s true in some settings, but the mapping between tiles and CTAs is technically abstracted by a [ TileScheduler](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L398-L404). For the best performance, you need to use the

[, which launches](https://github.com/Dao-AILab/flash-attention/blob/1ceaa984b2f348caea18b39a98458d33b4ea7a09/flash_attn/cute/tile_scheduler.py#L122)

`StaticPersistentTileScheduler`[at most one CTA per Streaming Multiprocessor](https://github.com/Dao-AILab/flash-attention/blob/1ceaa984b2f348caea18b39a98458d33b4ea7a09/flash_attn/cute/tile_scheduler.py#L162-L163)and then schedules tiles onto those SMs. This reduces CTA launch overhead and allows for more fine-grained concurrency (e.g. overlapping Epilogue warps for one tile with the Load and MMA warps for the next tile).
...
Also on the front of scheduling/simplification: only a single `leader_thread` issues the instruction. And we’re only working from a single warp. This is an important difference from performant Hopper MMAs, which were coordinated across an entire [warpgroup](https://modal.com/gpu-glossary/device-software/warpgroup).
...
At Modal, we’re building the cloud infrastructure that compute-intensive workloads like giant Transformers need. Our platform is used by companies like [Suno](https://modal.com/blog/suno-case-study), [Lovable](https://modal.com/blog/lovable-case-study), [Ramp](https://modal.com/blog/ramp-case-study), and [Substack](https://modal.com/blog/substack-case-study). We’re [hiring](https://modal.com/careers).
