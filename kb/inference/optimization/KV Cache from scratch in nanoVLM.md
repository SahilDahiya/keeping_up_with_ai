...
secondary_topics: []
summary: Implements KV caching from scratch in nanoVLM, explaining prefill vs decode,
  how cached keys/values remove redundant attention recomputation, and the code changes
  to the attention block and generation loop; yields a 38% generation speedup.
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:09:01Z'
classifier: claude
taxonomy_rev: 1
words: 1481
content_sha256: c27b5ff2bde22b9ebeb9d4cbc26418042d68bfcca8518619661fb28796b954fc
---
...
# 
	...
	](https://huggingface.co#kv-cache-from-scratch-in-nanovlm)
	
		KV Cache from scratch in nanoVLM
	

 [Update on GitHub](https://github.com/huggingface/blog/blob/main/kv-cache.md)

[  Upvote 120 ](https://huggingface.co/login?next=%2Fblog%2Fkv-cache)
...
- To generate token , the model must consider the entire sequence from  to . From the first instance in the above example  would be `the`, while all the previous tokens to would be`[What, is, in]`.
...
- [Revisiting the Transformer Architecture](https://huggingface.co#revisiting-the-transformer-architecture)
- [Where Redundancy Creeps In](https://huggingface.co#where-redundancy-creeps-in)
- [How KV Caching Fixes It](https://huggingface.co#how-kv-caching-fixes-it)
- [KV Caching in nanoVLM: From Theory to Practice](https://huggingface.co#kv-caching-in-nanovlm-from-theory-to-practice)
- [Summary: Why KV Caching Matters](https://huggingface.co#summary-why-kv-caching-matters)

## 
	[
		
	](https://huggingface.co#revisiting-the-transformer-architecture)
...
### 
	...
	](https://huggingface.co#self-attention-computation)
...
## 
	...
	](https://huggingface.co#where-redundancy-creeps-in)
...
## 
	...
	](https://huggingface.co#how-kv-caching-fixes-it)
...
## 
	...
	](https://huggingface.co#kv-caching-in-nanovlm-from-theory-to-practice)
...
### 
	...
	](https://huggingface.co#1-updating-kv-cache-in-the-attention-block)
...
### 
	...
	](https://huggingface.co#2-tracking-cache-across-layers)
...
### 
	...
	](https://huggingface.co#3-prefill-vs-decode-in-the-generation-loop)
...
### 
	...
	](https://huggingface.co#summary-of-changes)
...
|---|---|---|
| `LanguageModelGroupedAttention.forward` | Recomputes , , on every step | Uses and updates KV cache | 
| `LanguageModel.forward` | No memory of previous state | Tracks per-layer KV cache, handles `start_pos` | 
| `VisionLanguageModel.generate` | One-phase generation loop | Split into prefillanddecodephases | 

## 
	[
		
	](https://huggingface.co#summary-why-kv-caching-matters)
