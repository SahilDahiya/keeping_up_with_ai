...
fetched: '2026-07-11T04:17:29Z'
classifier: codex
taxonomy_rev: 1
words: 1252
content_sha256: e129735e7452f10adf694adde949dfbdadfc1e1607aa6f86c465a29df2e67f08
triage: keep
skip_reason: null
---

# FireAttention: serving open models faster with quantization

[Mixtral](https://mistral.ai/news/mixtral-of-experts/) has recently made waves in the AI community as the first OSS model trained on trillions of tokens to support 'mixture of experts' (MoE), which has promising features to speed up both training and inference.

Fireworks AI was the first platform to [host Mixtral, even before its public release](https://twitter.com/thefireworksai/status/1735202209049268414).
...
When measuring model quality, we will focus on language understanding. We'll use the [MMLU](https://arxiv.org/pdf/2009.03300.pdf) metric, which has enough test dataset examples (over 14K) and on which the Mixtral model performs quite well (70.6% accuracy), but not too well. This allows us to make **any quantization error quite visible** in the resulting accuracy numbers.
...
Many integer quantization methods for LLMs emerged. We've experimented with a few following [Huggingface's recommendation](https://huggingface.co/blog/mixtral). We also evaluated our FP8 implementation.

Our FP8 implementation runs 3 experts per each token (as opposed to the default 2). [See this post](https://twitter.com/FireworksAI_HQ/status/1737232544687665500) about concrete reasoning details.

We've measured the MMLU metric using a standard 5-shot template against the base Mixtral model.

- •GPTQ models were obtained from [GPTQ 8 bits](https://huggingface.co/TheBloke/Mixtral-8x7B-v0.1-GPTQ/tree/gptq-8bit-128g-actorder_True)and[GPTQ 4 bits](https://huggingface.co/TheBloke/Mixtral-8x7B-v0.1-GPTQ/tree/main)checkpoints.
- •LLM.int8() was obtained from the [original model](https://huggingface.co/mistralai/Mixtral-8x7B-v0.1)by passing load_in_8bit=True,dtype=float16
- •QLoRA 4 bit version was obtained by passing load_in_4bit=True,bnb_4bit_compute_dtype=float16 to the model constructor.

While LLM.int8() (and to some extent QLoRA) match the original model's quality, none of the integer quantization methods above provide any inference speed-ups, especially when running with batch sizes more than 1. Some good analysis is [done here](https://huggingface.co/Qwen/Qwen-72B-Chat-Int8#%E6%8E%A8%E7%90%86%E9%80%9F%E5%BA%A6%E5%8F%8A%E6%98%BE%E5%AD%98%E4%BD%BF%E7%94%A8-inference-speed--gpu-memory-usage).

Other methods like [SmoothQuant](https://arxiv.org/pdf/2211.10438.pdf) and [AWQ](https://arxiv.org/pdf/2306.00978.pdf) try to improve a model's performance, but still fall short, especially during generation. The fundamental issue of integer quantization still remains. And the issue is that LLM activations do not have uniform distribution, thus posing a challenge to integer methods.
...
[ZeroQuant-FP](https://arxiv.org/pdf/2307.09782.pdf) (Wu, X. et al. 2023) *Notably, FP8 activation exceeds INT8, especially in larger models. Moreover, FP8 and FP4 weight quantization are either competitive with or surpass their INT equivalents.*

[Which GPU(s) to Get for Deep Learning](https://timdettmers.com/2023/01/30/which-gpu-for-deep-learning/) (Dettmers T. 2023) *We can see that bit-by-bit, the FP4 data type preserve more information than Int4 data type and thus improves the mean LLM zeroshot accuracy across 4 tasks.*
...
We could not come by a working implementation of FP8 in Mixtral in OSS ([TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM) supports Mixtral and separately FP8 on selected few models, but FP8 doesn't work with Mixtral currently). Instead we are going to compare our FP16 with our FP8 implementation and will also cross-check with a very well-known OSS alternative [vLLM](https://github.com/vllm-project/vllm) for FP16 numbers.
...
Check out [Fireworks GenAI Platform](https://fireworks.ai/) if you want to get SOTA LLM performance including FP8-based FireAttention, accessible via our API endpoints. [Contact us](https://fireworks.ai/company/contact-us) for enterprise plan and deployment with the best speed and cost to serve catered for your workload.

If you are interested in deep system work and driving foundation model optimization innovation, please send your resume to [[email protected]](https://fireworks.ai/cdn-cgi/l/email-protection#4a292b382f2f38390a2c23382f3d25382139642b23). We are hiring system engineers, performance engineers and AI researchers. Check out all openings on our [Careers](https://fireworks.ai/company/careers) page.
