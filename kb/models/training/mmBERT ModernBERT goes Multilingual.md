...
summary: mmBERT is a ModernBERT-style multilingual encoder trained on 3T+ tokens across
  1,800+ languages using a three-phase schedule with an inverse masking-ratio decay
  and 'annealed language learning' that progressively adds low-resource languages
  late in training. Beats XLM-R and mGTE on multilingual retrieval and classification
  while running significantly faster.
triage: null
skip_reason: null
source: huggingface
url: https://huggingface.co/blog/mmbert
author: Marc Marone; Orion Weller; William Fleshman; Eugene Yang; Dawn Lawrie; Ben
  Van Durme
published: '2025-09-09'
fetched: '2026-07-14T22:08:55Z'
classifier: claude
taxonomy_rev: 1
words: 2918
content_sha256: 2e27670fc99b85c97911ac2402188496c814cbaad222dab53555d74135bbc3e1
---

# mmBERT: ModernBERT goes Multilingual

Fill-Mask •  Updated   •  473k  •  221  

#### jhu-clsp/mmBERT-base

![](https://cdn-avatars.huggingface.co/v1/production/uploads/1631039662102-6137ad94501f80a6f6e1eac9.png) 
...
If you are interested in trying out the models yourself, some example boilerplate is available [at the end of this blogpost!](https://huggingface.co#usage-examples)
...
**English Performance**: On the English GLUE benchmark ([Table 1](https://huggingface.co#table1)), mmBERT base achieves strong performance, substantially outperforming other multilingual models like XLM-R (multilingual RoBERTa) base and mGTE base, while remaining competitive to English-only models despite less than 25% of the mmBERT training data being English.

**Multilingual Performance**: mmBERT shows significant improvements on XTREME benchmark compared to XLM-R as demonstrated in [Table 2](https://huggingface.co#table2). Notable gains include strong performance on XNLI classification, substantial improvements in question answering tasks like TyDiQA, and competitive results across PAWS-X and XCOPA for cross-lingual understanding.
...
**English Retrieval**: Even though mmBERT is designed for massively multilingual settings, in the MTEB v2 English benchmarks ([Table 3](https://huggingface.co#table3)), mmBERT shows significant gains over previous multilingual models and even ties the capabilities of English-only models like ModernBERT!

**Multilingual Retrieval**: mmBERT shows consistent improvements on MTEB v2 multilingual benchmarks compared to other models ([Table 4](https://huggingface.co#table4)).

**Code Retrieval**: Due to the modern tokenizer (based on Gemma 2) mmBERT also shows strong coding performance ([Table 5](https://huggingface.co#table5)), making mmBERT suitable for any type of textual data. The only model that outperforms it is EuroBERT, which was able to use the non-publicly accessible Stack v2 dataset.
...
**Dramatic Performance Gains**: Testing on TiQuaD (Tigrinya) and FoQA (Faroese), we observed substantial improvements when these languages were included in the decay phase, as shown in [Figure 2](https://huggingface.co#figure2). The results demonstrate the effectiveness of our progressive language learning approach.
...
**Throughput Performance**: mmBERT processes text significantly faster than existing multilingual models across various sequence lengths, as demonstrated in [Figure 3](https://huggingface.co#figure3). Both the small and base models show substantial speed improvements over previous multilingual encoders.
...
- +19
 ...
 July 8, 2025 can someone point me to how can i use the mmBERT model for BERTscore , a translation eval metric

Hi [@StephennFernandes](https://huggingface.co/StephennFernandes) ! It's been a while since I used BERTScore, but wherever you see another BERT model (or XLM-R or similar) feel free to replace the model name with mmBERT. I would suggest checking out [https://github.com/Tiiiger/bert_score](https://github.com/Tiiiger/bert_score) for more details and perhaps raising an issue there if it's unclear.
