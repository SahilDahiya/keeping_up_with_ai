...
subtopic: training
secondary_topics: []
summary: 'The full engineering blueprint for SmolLM3-3B: 11T tokens over a three-stage
  pretraining data mixture, NoPE + YaRN for 128k context, and a dual-mode (think /
  no_think) hybrid reasoning instruct model — beating Llama-3.2-3B and Qwen2.5-3B,
  with exact data mixes and the mid-training/APO alignment recipe published.'
triage: null
skip_reason: null
source: huggingface
url: https://huggingface.co/blog/smollm3
author: Elie Bakouch; Carlos Miguel Patiño; Anton Lozhkov; Edward Beeching; Aymeric
  Roucher; Nouamane Tazi; Aksel Joonas Reedi; Guilherme Penedo; Hynek Kydlicek; Clémentine
  Fourrier; Nathan Habib; Kashif Rasul; Quentin Gallouédec; Hugo Larcher; Mathieu
  Morlon; Joshua; Vaibhav Srivastav; Xuan-Son Nguyen; Colin Raffel; Lewis Tunstall;
  Loubna Ben Allal; Leandro von Werra; Thomas Wolf
published: '2025-07-08'
fetched: '2026-07-14T22:08:59Z'
classifier: claude
taxonomy_rev: 1
words: 4685
content_sha256: d9e6fee791159b7048c463ba5bfb5343c1c0a013bb2cd7b7bfa1b1266c877a38
---

# SmolLM3: smol, multilingual, long-context reasoner

Text Generation •  3B • Updated   •  720k  •  982  
...
SmolLM3 both changed the architecture and data mixture over its predecessors. Let’s have a look at the architecture and training configurations first!
 ...
 ![](https://huggingface.co/datasets/HuggingFaceTB/images/resolve/main/smollm3/image%20(18).png)
...
- **Stage 1: Stable phase (0T → 8T tokens)**This foundation stage establishes strong general capabilities with our core dataset mixture:- Web: 85% (12% multilingual) - FineWeb-Edu, DCLM, FineWeb2 and FineWeb2-HQ
...
- **Stage 2: Stable phase (8T → 10T tokens)**We introduce higher quality math and code datasets while maintaining good web coverage:- Web: 75% (12% Multilingual)
...
- **Stage 3: Decay Phase (10T → 11.1T tokens)**The final stage further upsamples math and code data- Web: 63% (12% Multilingual)
...
`temperature=0.6`and`top_p=0.95`in the sampling parameters.
...
```
 ...
 Text Generation •  3B • Updated   •  116k  •  164 

 Text Generation •  33B • Updated   •  6.16M  •  715 

 Viewer • Updated  •  3.5B •  362k  •  1.19k 

 Updated   •  97  •  7 

 Viewer • Updated  •  273k •  2.49k  •  26 

 Viewer • Updated  •  3.91M •  4.79k  •  686 

 Viewer • Updated  •  1.2M •  16.4k  •  248 

 Paper • 2402.13991 • Published  •  1 

 Paper • 2501.18795 • Published  •  13 
...
- +2
 ...
 September 9, 2025 llmnlpcommunity
...
Definitely adding to the Weekend read..!
...
The model doesn't fit into each gpu for training (without zero/activation recomputation) and TP=2 didn't impact much the throughput so we choose this solution.

You can find more info on distributed pre-training here: [https://huggingface.co/spaces/nanotron/ultrascale-playbook](https://huggingface.co/spaces/nanotron/ultrascale-playbook)
...
Probably unrelated question, the [graph](https://huggingface.co/datasets/HuggingFaceTB/images/resolve/main/smollm3/image%20(17).png) shows Qwen3 1.7B has 2B parameters. Is it correct?

•

 Model page show that's why we made this msitake![Capture d’écran 2025-07-09 à 22.23.11.png](https://cdn-uploads.huggingface.co/production/uploads/651e96991b97c9f33d26bde6/pHw0DU0YrM12-lpw7ZDty.png)


But it's because of the embedding parameter count twice, thanks for noticing (the model have tied embedding iirc)

This [graph](https://huggingface.co/datasets/HuggingFaceTB/images/resolve/main/smollm3/image%20(18).png) shows the GQA has more query heads than key-value heads, which contradicts the original design. Is it intentional?

no it's not we use classical GQA, it's a typo sorry for this!

World needs heroes like you!!
...
Here's the [paper](https://arxiv.org/abs/2505.23735) for anyone interested: 
...
We released all the datasets we used for training in [SmolTalk2](https://huggingface.co/datasets/HuggingFaceTB/smoltalk2). The dataset card details which datasets were generated using Qwen3-32B.
...
Hi HF team, this SmolLM3 post got me curious about why you chose APO over GRPO, so I dove into comparing approaches across SmolLM3, Tulu3, and DeepSeek-R1. Ended up creating a visual guide to help navigate the post-training landscape [on 🤗 space](https://huggingface.co/spaces/bird-of-paradise/post-training-techniques-guide). 
...
Yes the wandb logs are here: [https://wandb.ai/huggingface/SmolLM3-training-logs](https://wandb.ai/huggingface/SmolLM3-training-logs) . They were released along with the intermediate checkpoints [https://x.com/eliebakouch/status/1947314495103160458](https://x.com/eliebakouch/status/1947314495103160458)

•

 Hi Hugging Face team! I noticed there isn't any specific details about the tokenizer implementation for multilingual support. Could you share some insights about your tokenization approach for handling multiple languages? I'm curious about whether you used a unified tokenizer or language-specific ones, which algorithm did you use to train the tokenlizer, and if there were any special techniques involved. Thanks!
...
Do you plan to share the rest of the datasets (mid-training, and post-training)?

This comment has been hidden (marked as Resolved)        

      ![image.png](https://cdn-uploads.huggingface.co/production/uploads/68b2a4157f881fc640ba7d80/icMJEzqt0YdnJw6wB8OEg.png)


it says beta1 0.8 in the picture but at somewhere of "Pretraining" it says "and the AdamW optimizer (beta1: 0.9, beta2: 0.95) with weight decay of 0.1"
...
GitHub: [https://github.com/mikecastrodemaria/SmolLM3-M2-Interface-Multimodale](https://github.com/mikecastrodemaria/SmolLM3-M2-Interface-Multimodale)
...
I can't find StarCoder2 pull request subset in the-stack-v2 dataset. Could you please show me the address?

Great article and really amazing job! Am I right that such a huge set of GPUs is needed to run a kind of genetic hyper parameters search?
