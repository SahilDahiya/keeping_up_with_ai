---
title: 'Kimi K2.7 Code on Fireworks: Better Agents, Lower Cost per Task, Available
  Day-0 | Fireworks'
kind: blog
topic: models
subtopic: reasoning
secondary_topics:
- agents/tool-use
summary: Kimi K2.7 Code uses roughly 30% fewer reasoning tokens than K2.6 while scoring
  higher on coding evals (+21.8% Kimi Code Bench v2, +11% Program Bench), illustrating
  that reasoning-token economy, not just raw benchmark score, matters for agentic
  coding workloads that fire off dozens of model calls per task.
triage: null
skip_reason: null
source: fireworks
...
fetched: '2026-08-12T06:30:19Z'
classifier: claude
taxonomy_rev: 2
words: 746
content_sha256: 67e08da033edbf706f2cb11421bad8c9d9aa7cb83d738225f8d3a88303646d55
---

# Kimi K2.7 Code on Fireworks: Better Agents, Lower Cost per Task, Available Day-0 | Fireworks
...
Model details: [https://fireworks.ai/models/fireworks/kimi-k2p7-code](https://fireworks.ai/models/fireworks/kimi-k2p7-code)

Model path: `accounts/fireworks/models/kimi-k2p7-code`

Here's the bit we find interesting.

The number that caught our eye isn't on the benchmark table. K2.7 Code uses **roughly 30% fewer reasoning tokens than K2.6**, and it still scores higher on coding evals:
...
Kimi K2.7 Code runs on Fireworks serverless, with [three serving options](https://fireworks.ai/blog/serverless-2) built for bursty agentic traffic:

- •**Standard** is the elastic, pay-per-token default. Every existing API call works as-is.
- •**Priority** is for traffic that cannot afford to be shed during peak congestion. Set`service_tier: "priority"` and your requests get stronger admission control on the shared fleet, at roughly 1.5x Standard pricing. Reliability, not speed, is the knob.
...
Try it in the [model playground](https://fireworks.ai/models/fireworks/kimi-k2p7-code), or read the [serverless docs](https://docs.fireworks.ai/guides/serverless-products) to configure Priority.
