...
secondary_topics:
- industry/trends
summary: A year-in-review of vision language models covering new model classes (any-to-any,
  reasoning VLMs, small on-device VLMs, MoE VLMs), multimodal RAG with ColPali-style
  late-interaction retrievers, VLM agents for GUI/computer use, video understanding,
  and how alignment/benchmarks for VLMs have evolved. Names the specific models and
  techniques behind each shift.
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:11:16Z'
classifier: claude
taxonomy_rev: 1
words: 5041
content_sha256: 8bfad9b39df75b196cbb601b602f8776c98d65d20ed6ba8133662e1df2df14f1
---

# Vision Language Models (Better, faster, stronger)

Image-Text-to-Text •  73B • Updated   •  837  •  610  
...
- [New Model Trends](https://huggingface.co#new-model-trends)
- [Specialized Capabilities](https://huggingface.co#specialized-capabilities)
- [Multimodal Agents](https://huggingface.co#multimodal-agents)
- [Video Language Models](https://huggingface.co#video-language-models)
- [New Alignment Techniques for Vision Language Models](https://huggingface.co#new-alignment-techniques-for-vision-language-models)
- [New Benchmarks](https://huggingface.co#new-benchmarks)
- [Extra: Our Model Picks](https://huggingface.co#extra-our-model-picks)
- [Useful Resources](https://huggingface.co#useful-resources)
...
The authors also released an instruction fine-tuned version called

[Kimi-VL-A3B-Instruct](https://huggingface.co/moonshotai/Kimi-VL-A3B-Instruct).

The model can take in long videos, PDFs, screenshots and more. It has agentic capabilities as well. ![kimi-vl](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/vlm2/kimi-vl.png)
...
To get a nice understanding of MoEs we recommend reading

[this fantastic article](https://huggingface.co/blog/moe).
...
A traditional PDF parser consists of multiple elements to preserve the structure and visual elements in the document, such as *layout*, *tables*, *images*, *charts,* all rendered into a markdown. But this setup can be hard to maintain.![Traditional Parsing](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/vlm2/rag-1.png)
...
Taken from ![ColPali](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/vlm2/colpali.png)

[the paper](https://arxiv.org/pdf/2407.01449)
...
Most vision language models these days can handle videos, because videos can be represented as a sequence of frames. However, video understanding is tricky because of the temporal relationship between frames and the large amount of frames, so different techniques are used to select a representative set of  video frames.![Video LMs](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/vlm2/video.png)
...
- Minimal training of VLM with vanilla PyTorch: [GitHub - huggingface/nanoVLM: The simplest, fastest repository for training/finetuning small-sized VLMs.](https://github.com/huggingface/nanoVLM)
 ...
 Image-Text-to-Text •  4B • Updated   •  6.76M  •  674 

 Any-to-Any •  Updated   •  12.4k  •  3.63k 

 Image-Text-to-Text •  27B • Updated   •  4.67k  •  387 

 Image-Text-to-Text •  4B • Updated   •  2.34M  •  1.41k 

 Image-Text-to-Text •  16B • Updated   •  357k  •  273 

 Image-Text-to-Text •  16B • Updated   •  146k  •  449 

 Any-to-Any •  9B • Updated   •  436k  •  1.29k 

 Image-Text-to-Text •  2B • Updated   •  1.99M  •  1.43k 

 Viewer • Updated  •  83.1k •  625  •  17 

 Preview • Updated   •  1.38k  •  215 
...
- +4
 ...
 June 26, 2025 multimodalnlpcommunity

  74

 April 16, 2026 Overall, this is a really great article that gives a primer on VLMS. I just wanted to chime in and say that Florence2 is also a really good model and one of my favourites for starting with unlabeled datasets. Another shout out to Moondream2 as well, which I find runs quite fast even on CPU/MPS

•

 hey Harpreet, thanks a lot! We wanted to cover primarily what came out after last April-May and latest developments, we'll give a shoutout to it. We already cover moondream2 in this one: [https://huggingface.co/blog/vlms](https://huggingface.co/blog/vlms)
...
Are you sure `google/gemma-3-1b-it` is multimodal ?

•

 We have fixed with 4B, sorry you're correct! the snippet actually belongs to 4b and we should've linked to 4b

GREAT piece ! Really AMAZING !

Well written & structured (as usual from you guys) and serves as a reference to get back to from time to time ...

Thanks folks

I hope it was useful!

Fantastic recap, thanks for the work!

However I still prefer Florence2 and Moondream in production due to their smaller sizes and wide range of supported output formats 👌🏻

•

 hello Fatih, teşekkürler! We wanted to cover primarily what came out after last April-May and latest developments, we'll give a shoutout to it. We have already covered them in this blog [https://huggingface.co/blog/vlms](https://huggingface.co/blog/vlms)

I agree with Kimi-VL, I wonder can we get the GGUF version since llama.cpp supports vision as well

Thank you for the great article. There seems to be an issue in the Multimodal RAG section where the same graphic is displayed twice?![grafik.png](https://cdn-uploads.huggingface.co/production/uploads/635a7dadbd6f7742b4c39d60/q06gfzNWt1mhiI1Ydhj50.png)


Thanks for reporting! It’s now updated 🙌

Thank you for the great article. It was very informative and well-organized> I wonder why there was no mention of InternVL. When comparing smaller VLM, InternVL does pretty well afaik.

thank you so much for your kind words! I think in general Qwen-VL variants (inc InternVL) do well yes :)

Thank you so much for the helpful clarification. I used Kimi-VL in a tree recognition demo by chance, and I was genuinely impressed by how well it performed. Now things are starting to make a lot more sense. 😊

very happy it's of use to you, indeed I love Kimi-VL as well! super impressive with the size etc especially :)

Looking forward to blog post on truly multimodal models (Omni models) in around 6 months :)

Fantastic overview of the VLM landscape! One recent development worth noting is the Skywork R1V series, which demonstrates impressive multimodal reasoning capabilities.

Skywork R1V introduced efficient cross-modal transfer of reasoning LLMs to vision, while R1V2 further advances this with hybrid reinforcement learning (MPO + GRPO). R1V2 achieves state-of-the-art open-source results: 73.6 on MMMU, 62.6 on OlympiadBench, and 78.9 on AIME24 - often outperforming much larger models.

What's particularly interesting is R1V2's solution to the "vanishing advantages" problem in GRPO through their Selective Sample Buffer mechanism, which maintains training efficiency as model responses converge.

Both models are fully open-sourced and would fit well in your reasoning VLM section alongside Kimi-VL and QVQ. Thanks for this comprehensive resource!

It's a readable and excellent project for a beginner like me. By reading this, I can quickly hands on some ideas I wanna do.

Very happy it's useful for you! 💗

wow!! thx!! :)

Really insightful breakdown . What excites me the most about VLMs is how quickly they’re evolving from static perception tools into interactive learning systems. The way they’re now handling multimodal RAG and long video understanding feels very similar to how structured learning modules like LETRS help people break down and master complex subjects step by step.
