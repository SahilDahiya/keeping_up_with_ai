...
summary: Explains the RLHF pipeline from preference data through reward modeling and
  policy optimization, including why human feedback changes model behavior and where
  evaluation matters.
source: chip-huyen
url: https://huyenchip.com/2023/05/02/rlhf.html
author: Chip Huyen
published: '2023-05-02'
fetched: '2026-07-11T05:08:46Z'
classifier: codex
taxonomy_rev: 1
words: 4823
content_sha256: a913a228a3c66f906e1c45592adfceca07a7ff7c3272b91ec1c138a30d1bbaa5
---
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/1-chatgpt-training.png)

    If you squint, this above diagram looks very similar to the meme Shoggoth with a smiley face.

- The pretrained model is an untamed monster because it was trained on indiscriminate data scraped from the Internet: think clickbait, misinformation, propaganda, conspiracy theories, or attacks against certain demographics.
- This monster was then finetuned on higher quality data – think StackOverflow, Quora, or human annotations – which makes it somewhat socially acceptable.
- Then the finetuned model was further polished using RLHF to make it customer-appropriate, e.g. giving it a smiley face.

![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/2-shoggoth.jpg)
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/3-complete.gif)
...
![RedPajama vs. LLaMa data](https://huyenchip.com/assets/pics/rlhf/4-1t-tokens.png)
...
![We're at the risk of running out of Internet data](https://huyenchip.com/assets/pics/rlhf/5-internet-data.png)
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/7-sft-prompts.png)
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/8-labeler-degrees.png)
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/9-sft-rlhf.png)
...
Yoav Goldberg has an excellent note on the

[three hypotheses on why RLHF works](https://gist.github.com/yoavg/6bff0fecd65950898eba1bb321cfbd81).
...
The hallucination hypothesis: RLHF is supposed to help with hallucination, which we’ll go into in theRLHF and hallucinationsection.
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/11-graph-rm-loss.png)
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/12-ui.png)
...
![3 phases of ChatGPT development](https://huyenchip.com/assets/pics/rlhf/6-sft-rlhf.png)
...
-
    For each \(x_{RL}\), we use \(LLM^{RL}_\phi\) to sample a response: \(y \sim LLM^{RL}_\phi(x_{RL})\). The objective is computed as follows. Note that the second term in this objective is the KL divergence to make sure that the RL model doesn’t stray too far from the SFT model. \[\text{objective}_1(x_{RL}, y; \phi) = RM(x_{RL}, y) - \beta \log \frac{LLM^{RL}_\phi(y \vert x)}{LLM^{SFT}(y \vert x)}\]
-
    For each \(x_{pretrain}\), the objective is computed as follows. Intuitively, this objective is to make sure that the RL model doesn’t perform worse on text completion – the task the pretrained model was optimized for. \[\text{objective}_2(x_{pretrain}; \phi) = \gamma \log LLM^{RL}_\phi(x_{pretrain})\]
...
![RLHF objective function](https://huyenchip.com/assets/pics/rlhf/14-rlhf-objective.png) The objective function as written in the InstructGPT paper.
...
![RLHF makes hallucination worse](https://huyenchip.com/assets/pics/rlhf/10-hallucination.png)
