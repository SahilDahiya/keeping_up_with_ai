...
secondary_topics: []
summary: 'Introduces TRL''s RLOO (REINFORCE Leave-One-Out) trainer as a lighter alternative
  to PPO for online RLHF: it drops the value network and uses other samples in the
  batch as the baseline, cutting GPU memory and wall-clock time to convergence while
  matching PPO''s GPT-4-judged win rate and beating offline DPO.'
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:11:52Z'
classifier: claude
taxonomy_rev: 1
words: 2480
content_sha256: 380c5f85782806a74b25e28ea08c5952fb1a8054497bfd655a48a6e17676a3c8
---
...
# 
	...
	](https://huggingface.co#putting-rl-back-in-rlhf)
	
		Putting RL back in RLHF
	

 [Update on GitHub](https://github.com/huggingface/blog/blob/main/putting_rl_back_in_rlhf_with_rloo.md)

[  Upvote 111 ](https://huggingface.co/login?next=%2Fblog%2Fputting_rl_back_in_rlhf_with_rloo)

![Shengyi Costa Huang's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/62bfc80b54db40ca617ce53a/IZxafMqoYJNHwTuFt8AqF.png) 

  ![Arash Ahmadian's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/661b21f8ea926e8f86729e61/gM59piqaqjEu2IPbDvCbZ.jpeg) 

  [Arash AhmadianArashAhmadian    ](https://huggingface.co/ArashAhmadian)

![CohereForAI's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/660eb9ff338e9556c90a6bbc/9DrmMdvUZKoHP3hRTngvc.png)

[CohereForAI](https://huggingface.co/CohereForAI)
...
With RLOO, we bring Reinforcement Learning back into RLHF, enabling the community to explore online RL methods more easily. This is exciting because more and more studies have shown that online RL is more effective than offline methods such as DPO ([https://arxiv.org/abs/2402.04792](https://arxiv.org/abs/2402.04792), [https://arxiv.org/abs/2405.08448](https://arxiv.org/abs/2405.08448)). 
  ...
  ![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/win_rate_comparison.png?download=true) 


  ![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/memory_runtime_comparison.png?download=true) 
...
![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/image7.png?download=true)
...
![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/image4.png?download=true)


# 
	[
		
	](https://huggingface.co#how-rloo-works)
...
From here on, regular PPO and RLOO differ in approach. RLOO has several key ideas. First, it treats the ** entire model completion** as a single action, whereas regular PPO treats 

**as individual actions. Typically, only the EOS token gets a true reward, which is very sparse. Regular PPO would attribute a reward to the EOS token, whereas RLOO would attribute that EOS reward to the entire completion, as demonstrated below.**
...
# 
	...
	](https://huggingface.co#get-started-with-using-rloo-with-trl)
...
![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/image9.png?download=true)
...
![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/image2.png?download=true)


![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/image6.png?download=true)
...
![image_tooltip alt_text](https://huggingface.co/datasets/trl-internal-testing/example-images/resolve/main/blog/putting_rl_back_in_rlhf_with_rloo/image5.gif?download=true)


# 
	[
		
	](https://huggingface.co#how-we-implemented-rloo-trainer-in-trl)
	
		How we implemented RLOO Trainer in TRL
	

We implemented RLOO trainer based on our new experimental `PPOv2Trainer`, which is itself based on [https://arxiv.org/abs/2403.17031](https://arxiv.org/abs/2403.17031). Interestingly, our implementation of the RLOO trainer still uses the PPO loss. This is because the loss of REINFORCE is a special case of PPO ([https://arxiv.org/abs/2205.09123](https://arxiv.org/abs/2205.09123)). Note that even though the logprob is explicitly in the REINFORCE loss, it is also implicitly in the PPO loss. Seeing is believing, so let's demonstrate this with a simple example.
...
- **🚀Highly performant RLOO checkpoint: **The 6.9B checkpoint gets a 78.7% (k=2) preferred rate using GPT4 as a judge, which even exceeds the best-reported performance of 77.9% (k=4) and 74.2 (k=2) in the original [paper](https://arxiv.org/abs/2402.14740). This is a good sign that our RLOO training is working as intended.- The RLOO 1B checkpoint has a 40.1% win rate compared to the SFT checkpoint's 21.3% win rate. This is a good sign that the RLOO training is working as intended.
...
# 
	...
	](https://huggingface.co#numerical-stability-the-dark-side)
...
# 
	...
	](https://huggingface.co#acknowledgment-and-thanks)
