...
summary: In async RL the trainer must ship the full model to the inference engine
  every step (14 GB for a 7B, ~1 TB for a frontier model); TRL exploits the fact that
  ~99% of bf16 weights are bit-identical between consecutive optimizer steps and syncs
  only a sparse safetensors delta via a Hub bucket, cutting Qwen3-0.6B's per-step
  payload from 1.2 GB to 20-35 MB and enabling fully disaggregated training with no
...
author: Amine Dirhoussi; Quentin Gallouédec; Kashif Rasul; Lewis Tunstall; Edward
  Beeching; Albert Villanova del Moral; Leandro von Werra; Sergio Paniego
published: '2026-05-27'
fetched: '2026-07-14T22:21:45Z'
classifier: claude
taxonomy_rev: 1
words: 3683
content_sha256: 188d1d566e42ea08a95e507a3402f1509c21a0979966e3b75544da852985c1bd
---

# Shipping a Trillion Parameters With a Hub Bucket: Delta Weight Sync in TRL

Paper • 2602.03839 • Published   
...
[  Upvote 42 ](https://huggingface.co/login?next=%2Fblog%2Fdelta-weight-sync)

![Amine Dirhoussi's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/69a5684704dd904ad8fdf1c6/_AC_1juch3FyFgy7mHCYa.jpeg) 
  ...
  ![Edward Beeching's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/1644220542819-noauth.jpeg) 

  ![Albert Villanova del Moral's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/1606406298765-noauth.jpeg) 
...
[a TRL PR](https://github.com/huggingface/trl/pull/5417)that encodes just the changed elements as asparse safetensors file, uploads it to aHugging Face Bucket, and tells vLLM to fetch it. On Qwen3-0.6B, the per-step payload drops from 1.2 GB to20 to 35 MB.- The cherry on top: we ran a full disaggregated training where the
...
- **Signal**Send the bucket coordinates. vLLM downloads, applies, returns.- `/update_weights`.
...
- **Fixed anchor cadence.**We currently dump a full anchor every steps. An adaptive policy ("anchor when cumulative drift exceeds X") would cut anchor cost on long runs.
...
- **Hooking into the optimizer.**Our attempt at predicting the mask from alone gave low recall, which means the analytical bf16 threshold is doing something more subtle than the textbook formula suggests. We would love to hear from anyone who has cracked this.
