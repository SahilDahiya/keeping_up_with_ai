...
summary: Surveys 16 open-source async RL libraries across 7 axes (orchestration, rollout
  buffers, weight-sync protocols, staleness handling, partial rollouts, LoRA, distributed
  backends); the shared pattern is disaggregating inference and training GPU pools
  so neither idles, with Ray dominating orchestration (8/16) and NCCL broadcast the
  default weight transfer.
triage: null
skip_reason: null
source: huggingface
url: https://huggingface.co/blog/async-rl-training-landscape
author: Amine Dirhoussi; Quentin Gallouédec; Kashif Rasul; Lewis Tunstall; Edward
  Beeching; Albert Villanova del Moral; Nouamane Tazi; Leandro von Werra; Sergio Paniego
published: '2026-03-10'
fetched: '2026-07-14T22:21:51Z'
classifier: claude
taxonomy_rev: 1
words: 9685
content_sha256: 6ade22f9bd2a90c623ab09086c3f4836f5edabbd1b10cf3675236f90f4745840
---
...
The problem:In synchronous RL (reinforcement learning) training, data generation (model inference to create data samples) dominates wall-clock time -- a single batch of 32K-token rollouts on a 32B (32-billion parameter) model can takehours,while the GPUs used for training remain idle.
...
We surveyed 16 open-source librariesthat implement this pattern and compared them across 7 axes: orchestration primitives, buffer design, weight sync protocols, staleness management, partial rollout handling, LoRA support, and distributed training backends.
...
[here's the full comparison table](https://huggingface.co#4-global-overview-sixteen-libraries-at-a-glance)(no reading required, we won't judge).But seriously, if you stick around, you might learn a thing or two about why your GPUs are idle 60% of the time.


- [1. Motivation: From synchronous RL training to async architectures](https://huggingface.co#1-motivation-from-synchronous-rl-training-to-async-architectures)
- [2. Libraries Surveyed](https://huggingface.co#2-libraries-surveyed)
- [3. The Comparison Framework: Seven Axes](https://huggingface.co#3-the-comparison-framework-seven-axes)
- [4. Global Overview: Sixteen Libraries at a Glance](https://huggingface.co#4-global-overview-sixteen-libraries-at-a-glance)
- [5. The Next Wave: Design Implications](https://huggingface.co#5-the-next-wave-design-implications)- [5.1 Critic-Free Algorithms: Memory Freed, But Weight Sync Pressure Increases](https://huggingface.co#51-critic-free-algorithms-memory-freed-but-weight-sync-pressure-increases)
- [5.2 Process Rewards: A New Synchronisation Barrier](https://huggingface.co#52-process-rewards-a-new-synchronisation-barrier)
- [5.3 Multi-Agent Co-Evolution: The Straggler Problem Compounds](https://huggingface.co#53-multi-agent-co-evolution-the-straggler-problem-compounds)
- [5.4 Training-Inference Mismatch: The Deepseek v3.2 MoE Case Study](https://huggingface.co#54-training-inference-mismatch-the-deepseek-v32-moe-case-study)
- [5.5 Distillation: The Same Async Problem Under a Different Name](https://huggingface.co#55-distillation-the-same-async-problem-under-a-different-name)
 
- [6. Design Choices for TRL's Async Trainer](https://huggingface.co#6-design-choices-for-trls-async-trainer)
...
- **Generation**, calls- `model.generate()`(or forward requests to a vLLM server) to produce G completions per prompt. This is autoregressive and dominates wall-clock time.
...
| Tunix | [github.com/google/tunix](https://github.com/google/tunix) | 2,175 | |
...
| Pub/Sub Message Bus | Components are decoupled producers and consumers communicating through append-only streams or message queues. Not orchestration per se, a data transport layerbetween independently running pools. | PipelineRL (inter-pool: Redis `XADD`/`XREAD`streams for multi-node, append-only JSONL files for single-node) | Clean decoupling across pool boundaries without RPC. Does not manage process lifecycle, scheduling, or fault recovery; must be paired with another orchestration type. | 
...
- **Object store for zero-copy data transfer.**Rollout data can be large, tens of GB per batch for very long-context reasoning. Ray's shared-memory object store enables zero-copy transfer between actors on the same node, avoiding serialization overhead that usually comes with- `multiprocessing.Queue`approaches.
...
|---|---|---|
| Implicit continuation | PipelineRL | Sequences are never interrupted. Weights swap between forward passes; the sequence simply continues with new weights. Stored logprobs remain valid because training uses the recorded, not recomputed. | 
...
- **HuggingFace**(PipelineRL, SkyRL/FSDP, verifiers-rl, ROLL, OAT, Atropos): The most common choice. Standard checkpoint format (- `peft`- `adapter_model.safetensors`), compatible with any HF Transformers training loop. ZeRO-3 interactions require care: OAT, for example, needs to disable the fused LM head; ROLL must disable gradient checkpointing entirely.
...
`vlm_lora`, but MoE-specific expert LoRA is not documented. On the inference side, That covers the seven axes, each captures a different facet of the same underlying problem. Together, they give us a complete lens for comparing libraries. Time to put it all on one page.
...
| Tunix | Native Python (ThreadPoolExecutor + asyncio); JAX-native | vLLM, SGLang, JAX | Cross-mesh reshard | Depth bounding | ❌ No | JAX/XLA 2D mesh | 2D JAX mesh: FSDP + TP; no PP; TPU-primary | ✅ qwix / QLoRA (Adapter-only) | |
...
- **Async reward pipelines become necessary.**PRIME-RL runs reward scoring concurrently with training as part of its fully async Orchestrator-Trainer pipeline; the Orchestrator handles scoring while the Trainer performs backward and optimizer steps independently. For PRM-based methods, this pipelined reward computation is not optional; synchronous reward scoring will dominate training wall time.
- **The separate preprocessor pool becomes necessary**. Running reference logprobs computation and PRM scoring on a dedicated GPU tier, for example, pipelined between generation and training, is the correct architecture for dense credit assignment.
...
**Implications for library design:** Both Keep Routing and Keep Sampling Mask require the inference server to return *additional metadata* alongside token logprobs, routing decisions, and sampling masks. The current API contract between inference servers (vLLM, SGLang) and trainers is `(token_ids, logprobs, finish_reason)`. Extending this to `(token_ids, logprobs, finish_reason, expert_routing, sampling_mask)` is a breaking change to every library's data flow.
...
`model_version` (No Double-Buffering)
...
`packed=True` directly supports bucketed broadcast: it packs parameters into configurable-size `uint8` buffers (default 1 GB, double-buffered across CUDA streams) and broadcasts them via a dedicated NCCL communicator separate from the training process group. This eliminates the per-parameter call overhead that dominates naive broadcast, yielding a massive sync speedup.Beyond vLLM's built-in engine, we will explore high-performance weight packing libraries for more demanding scenarios:

- [Awex](https://github.com/inclusionAI/asystem-awex)
- [Mooncake Transfer Engine](https://github.com/kvcache-ai/Mooncake)- [checkpoint-engine](https://github.com/MoonshotAI/checkpoint-engine)- [NVIDIA's NIXL transfer library](https://github.com/ai-dynamo/nixl).
...
- +4
 ...
 May 27, 2026 announcementopen-sourcecommunity
...
- +16
 ...
 June 8, 2026 Quite the astute post showing the state of RL in the open-source ecosystem. Kudos to all the authors who researched and have presented this work. 🙌
...
Good point [@garora](https://huggingface.co/garora) . The idea was that this would be a living document with the libraries selected as the initial pass over the ecosystem. Would you like to contribute and add openrlhf to the mix? If so, you can press the "Update on GitHub" button right at the top!
