...
summary: ModernBERT (149M base / 395M large) modernizes the BERT encoder with 8192-token
  context, rotary embeddings, alternating global/local attention, GeGLU, unpadding
  and Flash Attention 2, trained on 2T tokens of web+code — a Pareto improvement over
  BERT/DeBERTa on both speed and accuracy and a slot-in replacement for retrieval
  and classification encoders.
triage: null
skip_reason: null
source: huggingface
url: https://huggingface.co/blog/modernbert
author: Benjamin Warner; Antoine Chaffin; Benjamin Clavié; Orion Weller; Oskar Hallström;
  Said Taghadouini; Alexis Gallagher; Raja Biswas; Faisal Ladhak; Tom Aarsen; Nathan
  Cooper; Griffin Adams; Jeremy Howard; Jonathan Whitaker; Iacopo Poli
published: '2024-12-19'
fetched: '2026-07-14T22:09:23Z'
classifier: claude
taxonomy_rev: 1
words: 4706
content_sha256: cc1057579a2756eeb3347c4cdb851038cf2d98c24de149550947082b6936741a
---

# Finally, a Replacement for BERT: Introducing ModernBERT

Fill-Mask •  0.1B • Updated   •  264  •  8  
...
`transformers`ModernBERT will be included in v4.48.0 of `transformers`. Until then, it requires installing transformers from main:
...
More bluntly, decoder-only models are *too big*, *slow*, * private*, and 
...
* Inference costs*: What the above suggests, is that on an inference-per-inference basis, there are many times more inferences performed per year on encoder-only models than on decoder-only or generative models. An interesting example is 
...
Even more surprising: since RoBERTa, there has been no encoder providing overall improvements without tradeoffs (fancily known as “* Pareto improvements*”): DeBERTaV3 had better GLUE and classification performance, but sacrificed both efficiency and retrieval. Other models, such as 
...
In practice, it looks like this:![](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/modernbert/modernbert_alternating_attention.png)
...
*LightOn sponsored the compute for this project on Orange Business Cloud Avenue.*
 ...
 Fill-Mask •  0.1B • Updated   •  9.07M  •  625 

 Text Classification •  0.1B • Updated   •  17.6k  •  222 

  0.1B • Updated   •  3.55M  •  364 

 Text Generation •  406B • Updated   •  173k  •  979 

 Text Generation •  71B • Updated   •  68.5k  •  1.52k 

 Fill-Mask •  Updated   •  2.66M  •  430 

 Fill-Mask •  0.1B • Updated   •  1.91k  •  54 

 Text Generation •  0.1B • Updated   •  209k  •  298 

 Sentence Similarity •  22.7M • Updated   •  254M  •  5.07k 
...
- +2
 ...
 July 16, 2025 Loved it, thanks for having done the work, explained it and provided the new BERT model.
...
[@erickrf](https://huggingface.co/erickrf)  That probably would have defied their initial intent of creating the most efficient BERT.  A multilingual version has been trained from the base [https://huggingface.co/answerdotai/ModernBERT-base/discussions/60](https://huggingface.co/answerdotai/ModernBERT-base/discussions/60), but it may not target your intended language.

•

 Well done! Just sharing [PangolinGuard](https://huggingface.co/blog/dcarpintero/pangolin-fine-tuning-modern-bert/), a fine-tuned model from [ModernBERT-large](https://huggingface.co/answerdotai/ModernBERT-large) aimed at implementing AI guardrails. Despite its small size, the model closely approximates the performance of Claude 3.7 and Gemini Flash 2.0 on a mixed benchmark (based on BIPIA, NotInject, Wildguard-Benign and PINT). I believe this could provide a lightweight, inexpensive approach for (i) adding custom, self-hosted safety checks, (ii) steering conversations to compliant topics, and (iii) mitigating risks when connecting AI pipelines to external services.

If someone wants to explore the use of ModernBERT for AI safety, feel free to get in touch:[📝 article](https://huggingface.co/blog/dcarpintero/pangolin-fine-tuning-modern-bert/) | [🤗 hf-space](https://huggingface.co/spaces/dcarpintero/pangolin) | [repo](https://github.com/dcarpintero/pangolin-guard) 

I was very happy to see that vLLM added support for Modernbert recently! Wrote a short blog guide on using that [here](https://danielvanstrien.xyz/posts/2025/vllm/modern_inference_modernbert.html). 

Hope that more inference engines adding support for ModernBERT leads to even more nice fine tuned models being shared!

Excellent blog post.

Has anyone tried using ModernBERT as a base for LayoutLM? That would be a pretty cool move :-)

ModernBERT's success is delightful, and its performance in downstream tasks is astonishing. However, it has only been adapted for English so far; we hope to see it adapted for Chinese soon.
