---
title: Is finetuning GPT4o worth it? — with Alistair Pullen, Cosine (Genie)
topic: models
subtopic: fine-tuning
secondary_topics:
- evals-observability/evaluation
summary: Cosine interview asking whether fine-tuning GPT-4o is worth it, with practical
  tradeoffs around data, evals, and cost.
source: latent-space
url: https://www.latent.space/p/cosine
author: Latent Space
published: '2024-08-22'
fetched: '2026-07-11T05:20:18Z'
classifier: codex
taxonomy_rev: 1
words: 15013
content_sha256: 411d8a46126e49dc0ce3a01f9e15ec66e1195b7127aec0766fc4d7cd59d8b796
---

# Is finetuning GPT4o worth it? — with Alistair Pullen, Cosine (Genie)

[Betteridge's law](https://en.wikipedia.org/wiki/Betteridge%27s_law_of_headlines) says no: with seemingly infinite flavors of RAG, and >2million token context + prompt caching from Anthropic/Deepmind/Deepseek, it's reasonable to believe that "in context learning is all you need".

But then there’s [Cosine Genie](https://cosine.sh/blog/genie-technical-report), the first to make a huge bet [using OpenAI’s new GPT4o fine-tuning](https://openai.com/index/gpt-4o-fine-tuning/) for code at the largest scale it has ever been used externally; resulting in what is now the #1 coding agent in the world according to SWE-Bench Full, Lite, and Verified:

**SWE-Bench** has been the most successful agent benchmark of the year, receiving honors at ICLR (our interview [here](https://www.latent.space/p/iclr-2024-benchmarks-agents)) and recently being [verified by OpenAI](https://openai.com/index/introducing-swe-bench-verified/). Cognition (Devin) was [valued at $2b](https://aibusiness.com/nlp/six-month-old-ai-startup-behind-devin-now-valued-at-2b) after reaching 14% on it. So it is very, very big news when a new agent appears to beat all other solutions, by **a lot**:

![Tech Report Eval Graph Tech Report Eval Graph](https://substackcdn.com/image/fetch/$s_!JL-X!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F857881b1-b37a-45f9-a851-10cc2f6cfc76_800x542.webp)

While this number is self reported[1](https://www.latent.space#footnote-1), it seems to be corroborated by OpenAI, who also award it [clear highest marks on SWE-Bench verified](https://openai.com/index/gpt-4o-fine-tuning/):

![](https://substackcdn.com/image/fetch/$s_!vfy4!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F875730e9-985c-45a3-8d67-18f6f542457a_939x827.png)

The secret is **GPT-4o finetuning on billions of tokens of synthetic data. **

- **Finetuning:**As OpenAI- [says](https://openai.com/index/gpt-4o-fine-tuning/):- *Genie is powered by a fine-tuned GPT-4o model*- **trained on examples of real software engineers at work**, enabling the model to learn to respond in a specific way. The model was also trained to be able to- **output in specific formats, such as patches**that could be committed easily to codebases.- Due to the scale of Cosine’s finetuning, OpenAI worked closely with them to figure out the size of the LoRA: - *“They have to decide*- **how big your LoRA adapter**is going to be… because if you had a really sparse, large adapter, you’re not going to get any signal in that at all. So they have to- **dynamically size**these things.”
- **Synthetic data**: we need to finetune on the process of making code work instead of only training on working code.- “… - *we*- **synthetically generated runtime errors**. Where we would intentionally mess with the AST to make stuff not work, or index out of bounds, or refer to a variable that doesn't exist, or errors that the foundational models just make sometimes that you can't really avoid, you can't expect it to be perfect.”

Genie also has a 4 stage workflow with the [standard LLM OS tooling](https://www.latent.space/p/q2-2024-recap) stack that lets it solve problems iteratively:

![](https://substackcdn.com/image/fetch/$s_!dmHR!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fcfa46253-49d4-46f5-8840-f253b7e49ccc_1135x693.png)
...
- Cosine Genie - [launch](https://x.com/alistairpullen/status/1822981361608888619?s=46&t=jDrfS5vZD4MFwckU5E8f5Q),- [technical report](https://cosine.sh/blog/genie-technical-report)

## Timestamps

- [00:00:00] - [Suno Intro](https://suno.com/song/09a13a53-cff0-4457-8e1d-3026f6c69513)
