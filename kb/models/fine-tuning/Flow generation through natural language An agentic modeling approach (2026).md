...
secondary_topics:
- agents/tool-use
summary: Shopify fine-tunes a model to generate Flow automations from natural language,
  arguing differentiation comes from proprietary merchant-interaction data and the
  training recipe rather than closed-model API access.
triage: null
skip_reason: null
source: shopify
...
fetched: '2026-07-15T00:53:08Z'
classifier: claude
taxonomy_rev: 2
words: 2626
content_sha256: b06de840541a861c08daf08d3e58cdd2c0b7df7ee2ae02a44bdab5c181954ec0
---

# Flow generation through natural language: An agentic modeling approach (2026)
...
We fine-tuned a tool-calling agent to turn natural language into a Shopify Flow for [Sidekick](https://www.shopify.com/ca/sidekick), our AI commerce assistant. It's 2.2x faster, 68% cheaper, and outperforms closed models.
...
We fine-tuned Qwen3-32B on this synthetic dataset and evaluated it against a benchmark of 300 hand-crafted examples covering the breadth of expected Flow usage. An [LLM evaluation framework](https://shopify.engineering/building-production-ready-agentic-systems) compares the generated workflow against the expected one for semantic correctness, and validates syntactic correctness programmatically.
...
Prior work has explored Python as an intermediate representation ([SPEAC](https://arxiv.org/pdf/2406.03636), [LLMLift](https://arxiv.org/pdf/2406.03003), [WorkflowLLM](https://arxiv.org/pdf/2411.05451)), but via prompting or without a round-trip transpiler. What distinguishes this approach is the full loop: fine-tuning on Python combined with a transpiler back to the production DSL, without changing any downstream systems.
...
The full pipeline, from data collection through training, evaluation, and deployment, runs on [Tangle](https://shopify.engineering/tangle), Shopify's open-source ML experimentation platform. Tangle composes each step into a single reproducible workflow with intelligent caching. Only the affected steps re-run when one part changes.
...
The system improves as production traffic shifts, freeing the team to focus on expanding coverage and fixing systematic gaps rather than hand-curating data. The approach is similar in spirit to Karpathy's [Autoresearch](https://shopify.engineering/autoresearch), an automated loop that evaluates, keeps what works, discards what doesn't, and iterates—but applied to production data curation rather than training code.
