...
- •**Cost Efficiency:** On identical tasks, V4 Pro costs**$2.50 per solved task** vs. K3’s**$4.91** (a**1.96x cost advantage** for V4 Pro). Across an 863-task diagnostic, that 2x ratio holds. You can run twice as many security scans for the same budget.
- •**Standalone Accuracy:** V4 Pro solves 462 of 920 tasks (**50.2%** ), costing**$1.31 per run** and**$2.60 per success** . Sub-scores hit**54.7%** on ARVO tasks and**48.7%** on OSS-Fuzz tasks.
- •**Where It Fails (The Harness Opportunity):** Once V4 Pro completes stage-1 validation (generating a valid proof-of-concept), it finishes the full patch**94.7% of the time** .
