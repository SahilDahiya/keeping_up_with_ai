...
Tiny train-inference numerical disagreements aren't benign noise: they quietly turn on-policy RL off-policy and can independently cause a run to **collapse**. For related background on inference nondeterminism, see [TML](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/); for the RL failure mode, see this [diagnosis](https://arxiv.org/abs/2605.14220). The clean answer isn't to correct the gap with importance sampling, which only piles on variance; it's to **erase** it. When training and inference are made bitwise consistent so the KLD is exactly **0**, RL trains in fewer steps and reaches higher reward ([vLLM x TorchTitan](https://vllm.ai/blog/2025-11-10-bitwise-consistent-train-inference)).

That's what Fireworks delivers on GLM 5.2: **batch-invariant serving with zero-KLD train-inference alignment.** The rollout engine returns bit-for-bit identical logits no matter the batch size, the concurrent load, or how many GPUs it's sharded across, and the trainer is held to that same bit-exact standard, so the **full train-rollout loop runs at zero KLD**. This is the guarantee frontier labs build in-house, and it's genuinely hard to hold across different engines, kernels, and parallelism layouts. Determinism across tensor-parallel sizes alone was an open research problem as recently as this year ([TBIK, ICML 2026](https://icml.cc/virtual/2026/poster/66224)).

The catch everyone else hits is speed: open-source deterministic modes typically run **35-60% slower** ([SGLang](https://lmsys.org/blog/2025-09-22-sglang-deterministic/)). Fireworks pays virtually none of that tax. The GLM trainer holds **around 3,500 tokens/sec per node, on par with the OSS TileLang implementation**, and layers the zero-KLD numerics on top rather than trading speed for them. One stack gives the SFT/DPO majority full throughput and gives RL teams zero-KLD numerics: frontier-lab infrastructure, delivered as a managed service.

1. **A frontier model, RL-ready.** GLM 5.2 is live for fine-tuning through the Fireworks Training API, with the full numerical foundation carried forward from GLM 5.1. The validated training shape is public today.
2. **The methods that matter.** SFT, DPO, and RL through the Training API; SFT and DPO on managed training. On-policy RL where the trainer and serving engine genuinely agree so your signal is real learning, not drift you clipped away.
3. **Reproducible, auditable inference.** Temperature-0 requests return the same answer regardless of server load so you get trustworthy evals, meaningful regression tests, and the reproducibility enterprise compliance demands.
4. **Fast where it counts.** A trainer tuned for SFT/DPO throughput, rollouts that generate about**1.8x faster on GLM 5.2 than GLM 5.1** (around 5,000 tokens/sec per node, per promotion-CI), and**zero-KLD** train-serve numerics, so RL is genuinely on-policy.
5. **Managed, self-service, and co-located.** Run the full loop on managed infrastructure with trainer and deployment co-located for fast weight sync, or drive longer runs yourself through the API.
...
To get started, [please reach out to our training team](https://fireworks.ai/contact-training) today, or dive straight in with our [managed training docs](https://docs.fireworks.ai/fine-tuning/managed-finetuning-intro).
