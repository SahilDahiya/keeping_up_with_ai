...
fetched: '2026-08-12T06:30:17Z'
classifier: null
taxonomy_rev: 2
words: 1225
content_sha256: 79cc23871167c672fff97c5bd2f38a6d779fbfcad2ab493d31751a080e195b84
---
...
[**Vercel** used our RL infrastructure](https://fireworks.ai/blog/vercel) to build a custom "Auto Fix" model for v0. The model checks the output stream for errors and self-corrects without a second pass, reaching a **93% error-free generation rate**, significantly outperforming closed frontier models, with a **40X improvement in end-to-end latency vs. the proprietary model it replaced** and over 8,000 characters per second throughput.
...
Most training services cap out at LoRA. LoRA is the right starting point: fast, cost-effective, and well-suited for rapid iteration. But [LoRA and full-parameter training learn in meaningfully different ways](https://fireworks.ai/blog/scaling-optimizing-frontier-model-training). LoRA learns less and forgets less. Full-parameter produces behavioral changes that adapter-based methods can't reach.

Fireworks Training now supports full-parameter training from Qwen3 8B on a single node to Kimi K2.5 (1T parameters) on 64x B200s. Getting there required solving distinct distributed systems problems at each scale: composable parallelism across FSDP, pipeline, context, and expert dimensions; Blackwell-native MXFP8 for expert computation; and streaming pipeline parallelism so RL rollout data trains as it arrives rather than waiting on batch accumulation. [More detail in the training infrastructure writeup.](https://fireworks.ai/blog/scaling-optimizing-frontier-model-training)
...
[The bottleneck is rarely the algorithm.](https://fireworks.ai/blog/fine-tuning-bottlenecks) It's integration friction and iteration speed. Start at whatever level of abstraction fits your team. Drop down when you need more control. Same platform, same deployment targets throughout.
...
Fireworks Training is in preview. [Start Here](https://fireworks.ai/contact-training)

Further reading: [Scaling and Optimizing Frontier Model Training](https://fireworks.ai/blog/scaling-optimizing-frontier-model-training) · [Frontier RL Is Cheaper Than You Think](https://fireworks.ai/blog/frontier-rl-is-cheaper-than-you-think) · [When Faster ≠ Identical: Numerical Pitfalls in MoE Models](https://fireworks.ai/blog/when-faster-not-identical-moe-numerics) · [The Fine-Tuning Bottleneck Isn't the Algorithm](https://fireworks.ai/blog/fine-tuning-bottlenecks)*
