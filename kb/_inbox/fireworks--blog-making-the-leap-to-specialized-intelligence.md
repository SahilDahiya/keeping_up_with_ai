...
1. **yes/no** questions - K3 wins
2. questions where **numbers** are the answer - K3 wins
3. questions where there’s a natural language answer - a tie
4. **coordinate** questions where the AI must draw a box around the item being asked for - Sol wins
...
1. Split the UIPad dataset into a training / testing set
  1. This way we can be more confident that the model can generalize its learnings from the training set to unseen data on the test set
2. Run Kimi K3 and GPT 5.6 Sol on the test set to get a baseline score
  1. Note beforehand, I was showing you scores on the **whole** dataset, not split up so we need to do this again to get a fair comparison
3. Note beforehand, I was showing you scores on the 
4. Train K3 on the training set and **not** the test set
  1. I used the Fireworks [Serverless Training API](https://docs.fireworks.ai/fine-tuning/training-api/serverless)
  2. I pointed my coding agent to our [cookbook’s training skill](https://github.com/fw-ai/cookbook) file and let it do the rest
5. I used the Fireworks 
6. Run the new trained model against the same test set to get a final score
...
Sinan OzdemirHead of AI Developer Education
