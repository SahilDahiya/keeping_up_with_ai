---
title: A practitioner's guide to testing and running large GPU clusters for training
  generative AI models
topic: infra-platform
subtopic: gpu-clusters
secondary_topics:
- models/fine-tuning
summary: Practical guide to testing and operating large GPU clusters for generative
  model training.
source: together
...
author: Ryan Lucchese; Niki Birkner; Yaron Hagai; Virginia Adams
published: '2024-08-13'
fetched: '2026-07-11T04:22:21Z'
classifier: codex
taxonomy_rev: 1
words: 2682
content_sha256: 0d38a6a974eb4d4e25c008b29425177c2f639cec13390dc7ba9a6872ab4969b6
triage: keep
skip_reason: null
---
...
The reliability of GPU clusters varies dramatically, ranging from minor issues to critical failures. Even industry giants like Meta have [reported](https://ai.meta.com/research/publications/the-llama-3-herd-of-models/) significant hardware challenges. During a 54-day training run of their Llama 3.1 model, “*GPU issues were the largest category, accounting for 58.7% of all unexpected issues*”.
...
The heart of GPU validation lies in the stress testing. For this, we utilize [DCGM Diagnostics](https://docs.nvidia.com/datacenter/dcgm/latest/user-guide/dcgm-diagnostics.html) from NVIDIA as well as [gpu-burn](https://github.com/wilicc/gpu-burn). DCGM will perform a number of tests including measuring power consumption and temperature while the GPU is under load. If any of the various test cases from DCGM don’t pass, we know we’ve likely got a problem. We generally run DCGM with [Apptainer](https://apptainer.org/). You can build the sif file like this:
...
If we have a positive result when evaluating each of the GPUs individually, then we need to make sure the GPUs can work together on a single machine. There are two main tools for this: NCCL tests and nvbandwidth.[NCCL tests](https://github.com/NVIDIA/nccl-tests) can test GPU to GPU communication over NVLink when run on a single machine, and we should see that for large message sizes, we approach the unidirectional performance of NVLink. If the performance is lower than expected, or we get some errors, we can quickly diagnose problems like a bad NVSwitch or down NVLinks. Similarly the nvbandwidth tool will measure copy performance from GPU to GPU.

The nvbandwidth tool can be built from source from the NVIDIA repository [https://github.com/NVIDIA/nvbandwidth](https://github.com/NVIDIA/nvbandwidth) and then run with default arguments. It will run a lot of tests, for many of which the results look like this, showing the speed of GPU to GPU memcpy:
...
For Machine Learning calculations, we are very interested in making sure that [GPUDirect RDMA](https://docs.nvidia.com/cuda/gpudirect-rdma/) is working optimally, and for this we again use NCCL tests, similar to validating NVLink. This time we will include multiple nodes in the NCCL test, starting with 2 nodes all the way up to the entire cluster. Generally we are looking for the all_reduce_perf test to show bandwidth around 92% of the theoretical maximum of the fabric: so around 370 GB/s on a 400GB/s fabric.
...
```
It is also important to validate Ethernet networks, for which we use [iperf3.](https://software.es.net/iperf/)
...
![__wf_reserved_inherit](https://cdn.prod.website-files.com/69654e88dce9154b5f12070c/699e0af80b270c56e85d7440_66ba82f57d75000a26a45009_66ba82e4265c8cc48fceed45_Screenshot%2525202024-08-12%252520at%2525202.46.46%2525E2%252580%2525AFPM.png)

![](https://cdn.prod.website-files.com/69654e88dce9154b5f12070c/699e0af80b270c56e85d744d_66ba604d966f4d5fc89d3c17_AD_4nXdEVQcGANCG_DRepXegQ004ECMKfnaurVxAgCT_darG3_n8n08jjbNTPiaDR_czwGvuWseCEbLbcGZRYS2w3UAwtRdQoh5tEV3BAGhjCSEEe3IbAerXuMvod0teSHpCSQ-o0SNGwxl1fFYY8OIW2DB221WQ.png)

![Side-by-side bar charts showing large computation times versus shorter data transfer and synchronizing times in microseconds.](https://cdn.prod.website-files.com/69654e88dce9154b5f12070c/699e0af90b270c56e85d7453_66ba604d79c2258b976841bd_AD_4nXc9CNfLPVOVDTtrDqh8HHocwWXfgJrJlvL9OiK6sSylJU_jDK0djgcktciCiqg-qhic_TTpH7cDo19zBW_OYDEtXznn-dIormkUE_qS2tRwJQH9u_w7bj2An3fVm2hZomOljoivfLNGsskN-8KnziIY0x7u.png)

Through this exercise we verify our cluster is able to achieve [reasonable MFU performance](https://pytorch.org/blog/maximizing-training/) and communication efficiency for models in the 1 to 10 billion parameter range. 
...
![Cluster-level metrics showing an issue with the average number of GPUs in the cluster.](https://cdn.prod.website-files.com/69654e88dce9154b5f12070c/699e0af80b270c56e85d7449_66ba604da59fdff6a59720ab_AD_4nXciIX2eYByObO15WDfpAsSh_LRPuetq8O80xUPqNtrUHU9I2VJsyBqub4WzpBBJq4cnUCM38s2ryFPaWSVrmFO7O_0Jj2qJKQZxHbOit8zezgLE3Wkg8w-em4kqWrOIGl-f86j_I0uxtwWnkiLIB7cZkrF0.png)

We can then use host-level metrics to pinpoint the server that has a bad GPU, indicating that a GPU “fell of the bus”.

![Host-level metrics showing a missing GPU.](https://cdn.prod.website-files.com/69654e88dce9154b5f12070c/699e0af80b270c56e85d743d_66ba604e6b1eea52d62df4cf_AD_4nXcDN_XcllNwWAZjvwDszlJDpxbAWACwwfyiQdAD1PXh6pbZ5rsVTZRy4W_UUgTAuFivInY7444bq8ETuB97kNAeWY8Fibr_D3PEbxumFynOEGEE2KbCv1PP-HkpGdYlZukg_ZHuZydtrDLWq2CIyflyanmQ.png)
...
![Host-level metrics showing GPU temperature and power draw.](https://cdn.prod.website-files.com/69654e88dce9154b5f12070c/699e0af90b270c56e85d7450_66ba604e6c54aa1e2cf3e5a7_AD_4nXdu12CO7cDZaKWwic2i5JbGm-I6DwDwfuCN7eirtwZ50Wx5DMZ5f4fjc6SoUOMv-12W1Jc6VgsZF004NCVyaZnqj1CJ1oZQOXXo0fguNzEnX-WHg3bKHcopfOCWFgvePOmtCIDbSH1XDr1eoGze7GDkxvdr.png)

Another interesting metric is DNS lookup errors. We noticed that DNS lookup errors affected training runs, so we quickly added this monitor to all of our infrastructure. We now monitor the ability of our servers to perform DNS lookups to interesting domains like R2 and S3 (for dataset download) and [wandb.ai](https://wandb.ai/) (for training with Weights & Biases).

![Host-level metrics showing DNS lookup errors.](https://cdn.prod.website-files.com/69654e88dce9154b5f12070c/699e0af80b270c56e85d7443_66ba604d081bbb4c6ee54750_AD_4nXegSg6PPao5Jh7o2Lkf1LKVSjcr9uLOf8_j2wdyK4uPJSuClc5dPI6eqxPhYQwt20POuE0nvwMS2-y_jU3uk41yHRabaT3EQ7y3kn8xiE-BH02D5OBnEQ1aujQDxaSYAe8IOVxztnH7TB7aAcM-mKpgepvT.png)

The DNS lookup error metric is a great example of Telegraf’s customizability. It is easy to use the [DNS Query plugin](https://github.com/influxdata/telegraf/blob/release-1.31/plugins/inputs/dns_query/README.md), with this simple config, to gather these metrics:
