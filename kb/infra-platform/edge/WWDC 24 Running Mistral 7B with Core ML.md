...
summary: 'Reproduces Apple''s WWDC''24 Mistral-7B Core ML demo: exporting a Swift-Transformers
  model, stateful KV cache, multifunction models for prefill vs extend, INT4 block-wise
  weight quantization, and running across CPU/GPU/ANE on Apple Silicon.'
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:06:06Z'
classifier: claude
taxonomy_rev: 1
words: 2708
content_sha256: d8bb9bc69d3e60febe523f7c100b1dd1f3724c5f4f1fd524e50290100c2a0edc
---

# WWDC 24: Running Mistral 7B with Core ML

Text Generation •  Updated   •  81  •  70  
...
Step 2: Download the converted Core ML models from 

`this Hugging Face repo``swift run transformers "Best recommendations for a place to visit in Paris in August 2024:" --max-length 200 Mistral7B-CoreML/StatefulMistralInstructInt4.mlpackage`Here are some of the most impactful Core ML features from WWDC’ 24 we will use to run Mistral 7B on a Mac.
...
- The model updates presented here are excellent for GPUs on Mac computers. Core ML can use the Neural Engine, which is particularly efficient on iPhones. Getting the most performance out of the Neural Engine requires some additional adaptations, which we plan to carry out on a few example models. This work will be based on the learnings discussed in this - [2022 (and still very relevant) article by Apple](https://machinelearning.apple.com/research/neural-engine-transformers). We won’t run Mistral 7B on iPhone, but there are several smaller models, like Apple’s OpenELM or DCLM that make for great candidates to explore!
