...
summary: End-to-end guide to building a React Native chat app that runs LLMs fully
  on-device via llama.rn/llama.cpp, covering how to pick mobile-viable models, what
  the GGUF quantization suffixes (Q2_K, Q4_K_M, Q8_0) actually trade off in size vs
  quality, and the Expo/native build plumbing.
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:05:44Z'
classifier: claude
taxonomy_rev: 1
words: 6288
content_sha256: c43c29b26e0322d80ad6ff9f9a73bb06dfb6819400fabe7f5370fd8856e98c21
---

# LLM Inference on Edge: A Fun and Easy Guide to run LLMs via React Native on your Phone!

Text Generation •  2B • Updated   •  162k  •  742  
...
- Use the search filters:- Visit the [GGUF models page](https://huggingface.co/models?library=gguf)
...
- **Node.js:**Node.js is a JavaScript runtime that allows you to run JavaScript code. It is essential for managing packages and dependencies in your React Native project. You can install it from- [Node.js downloads](https://nodejs.org/en/download).
...
- **If you are on macOS:**- For iOS: Install Xcode -> Open Developer Tools -> Simulator
...
- **If you are on Windows or Linux:**- For iOS: We need to rely on cloud-based simulators like [LambdaTest](https://www.lambdatest.com/test-on-iphone-simulator)and[BrowserStack](https://www.browserstack.com/test-on-ios-simulator)
- For Android: Install Java Runtime and Android Studio -> Go to Device Manager and Create an emulator
 
- For iOS: We need to rely on cloud-based simulators like 
...
- `ios/`- Contains native iOS project files
- **Purpose**: To build and run the app on iOS devices
 
- `node_modules/`- **Purpose**: Holds all npm dependencies used in the project
 
- `App.tsx`- The main root component of your app, written in TypeScript
- **Purpose**: Entry point to the app's UI and logic
 
- `index.js`- Registers the root component (`App`)
...
- **Clone the Repository**:- `git clone https://github.com/MekkCyber/EdgeLLM.git`
- **Navigate to the Project Directory**:- `cd EdgeLLMPlus #or cd EdgeLLMBasic`
- **Install Dependencies**:- `npm install`
- **Navigate to the iOS Folder and Install**:- `cd ios pod install`
- **Start the Metro Bundler**: Run the following command in the project folder (EdgeLLMPlus or EdgeLLMBasic):- `npm start`
- **Launch the App on iOS or Android Simulator**: Open another terminal and run:- `# For iOS npm run ios # For Android npm run android`
...
- **Chat-related**:- The conversation history (messages between user and AI)
- Current user input
 
- **Model-related**:- Selected model format (like Llama 1B or Qwen 1.5B)
...
- Fetch available GGUF models from [Hugging Face](https://huggingface.co/)
...
- When a user selects 'Llama-3.2-1B-Instruct', it maps to `medmekk/Llama-3.2-1B-Instruct.GGUF`
...
Let's create `src/api/model.ts` and copy the code from the [ src/api/model.ts](https://github.com/MekkCyber/EdgeLLM/blob/main/EdgeLLMBasic/src/api/model.ts) file in the repo. The logic should be simple to understand. The same goes for the progress bar component in the 

`src/components``View` where the width is the progress of the download.Now we need to test the `handleDownloadModel` function, let's add a button to the UI to trigger the function, and we will display the progress bar. This will be added under the `ScrollView` we added before.
...
- Press `j`in the Metro bundler terminal to launch Chrome DevTools
- Navigate to the "Sources" tab

3. Find your source files![alt text](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/llm-inferencen-on-edge/dev_tools.png)
...
- **Model Management:**- Allow users to delete models from the device
...
Happy coding! 🚀
 ...
 Text Generation •  0.5B • Updated   •  428k  •  201 

 Text Generation •  2B • Updated   •  686k  •  1.54k 

 Text Generation •  1B • Updated   •  1.73M  •  2.49k 

 Text Generation •  1B • Updated   •  9.93M  •  1.53k 
...
peftloraguide
 ...
 June 18, 2026 guidehubjobs

  11

 June 9, 2026 Very very cool work!

•

 Would be great for you to give us a better idea of size on disk for this. Uploading to Play/IOS store considerations. Maybe integration from pytorch into tensorflow to take advantage of the M chip in Pixel to really make this stuff scream, using a tool like google-ai-edge/ai-edge-torch. Great start.

Downloading the model after installation is a great idea, as it keeps the app lightweight and gives users the flexibility to choose the model that best suits their needs.

Hi can I convert VLMs into GGUF?

very helpfull , most of this small models are good in english only , you guys can implement a translation feature model like ML kit to translate to multiple language but i tried it the context is missing since i am translating word to word it miss context, any idea to do that ?
