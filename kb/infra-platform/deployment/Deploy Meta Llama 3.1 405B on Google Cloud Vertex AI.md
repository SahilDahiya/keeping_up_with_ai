...
secondary_topics:
- inference/serving
summary: 'Step-by-step deployment of Llama 3.1 405B (FP8 quantized) on Google Cloud
  Vertex AI with Hugging Face TGI on an A3 8xH100 node: registering the model, endpoint
  config, and running online inference with 128k context.'
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:09:51Z'
classifier: claude
taxonomy_rev: 1
words: 2620
content_sha256: d85117f785c2e0caac92484d7b1e4c965459b8aaf5b4dd8f77723d6b93d8109f
---
...
# 
	...
	](https://huggingface.co#deploy-meta-llama-31-405b-on-google-cloud-vertex-ai)
	
		Deploy Meta Llama 3.1 405B on Google Cloud Vertex AI
	

 [Update on GitHub](https://github.com/huggingface/blog/blob/main/llama31-on-vertex-ai.md)

[  Upvote 17 ](https://huggingface.co/login?next=%2Fblog%2Fllama31-on-vertex-ai)

![Alvaro Bartolome's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/60f0608166e5701b80ed3f02/H9LNnlf0W4fiOJhQMxnbW.jpeg) 

  ![Philipp Schmid's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/1624629516652-5ff5d596f244529b3ec0fb89.png) 

  ![Simon Pagezy's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/noauth/w3Z6xyKVBA6np65Tb16dP.jpeg) 

  ![Jeff Boudier's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/1605114051380-noauth.jpeg) 
...
In this blog you will learn how to programmatically deploy [ meta-llama/Meta-Llama-3.1-405B-Instruct-FP8](https://hf.co/meta-llama/Meta-Llama-3.1-405B-Instruct-FP8), the FP8 quantized variant of 

[, in a Google Cloud A3 node with 8 x H100 NVIDIA GPUs on Vertex AI with](https://huggingface.co/meta-llama/Meta-Llama-3.1-405B-Instruct)

`meta-llama/Meta-Llama-3.1-405B-Instruct`[Text Generation Inference](https://github.com/huggingface/text-generation-inference)(TGI) using the Hugging Face purpose-built Deep Learning Containers (DLCs) for Google Cloud.

Alternatively, you can deploy [ meta-llama/Meta-Llama-3.1-405B-Instruct-FP8](https://hf.co/meta-llama/Meta-Llama-3.1-405B-Instruct-FP8) without writing any code directly from the 

[Hub](https://huggingface.co/blog/google-cloud-model-garden#how-it-works---from-the-hub)or from

[Vertex Model Garden](https://console.cloud.google.com/vertex-ai/model-garden)!

This blog will cover:

- [Requirements for Meta Llama 3.1 Models on Google Cloud](https://huggingface.co#1-requirements-for-meta-llama-31-models-on-google-cloud)
- [Setup Google Cloud for Vertex AI](https://huggingface.co#2-setup-google-cloud-for-vertex-ai)
- [Register the Meta Llama 3.1 405B Model on Vertex AI](https://huggingface.co#3-register-the-meta-llama-31-405b-model-on-vertex-ai)
- [Deploy Meta Llama 3.1 405B on Vertex AI](https://huggingface.co#4-deploy-meta-llama-31-405b-on-vertex-ai)
- [Run online predictions with Meta Llama 3.1 405B](https://huggingface.co#5-run-online-predictions-with-meta-llama-31-405b)
- [Clean up resources](https://huggingface.co#6-clean-up-resources)
...
## 
	...
	](https://huggingface.co#introduction-to-vertex-ai)
...
## 
	...
	](https://huggingface.co#1-requirements-for-meta-llama-31-models-on-google-cloud)
...
## 
	...
	](https://huggingface.co#2-setup-google-cloud-for-vertex-ai)
...
```
Finally, as the Meta Llama 3.1 models are gated under the [ meta-llama organization in the Hugging Face Hub](https://hf.co/meta-llama), you will need to request access to it and wait for approval, which shouldn't take longer than 24 hours. Then, you need to install the 

`huggingface_hub` Python SDK to use the `huggingface-cli` to log in into the Hugging Face Hub to download those models.```
pip install --upgrade --quiet huggingface_hub
```
Alternatively, you can also skip the `huggingface_hub` installation and just generate a [Hugging Face Fine-grained Token](https://hf.co/settings/tokens) with read-only permissions for [ meta-llama/Meta-Llama-3.1-405B-Instruct-FP8](https://hf.co/meta-llama/Meta-Llama-3.1-405B-Instruct-FP8) or any other model under the 

[, to be selected under e.g.](https://hf.co/meta-llama)

`meta-llama` organization`Repository permissions -> meta-llama/Meta-Llama-3.1-405B-Instruct-FP8 -> Read access to contents of selected repos`. And either set that token within the `HF_TOKEN` environment variable or just provide it manually to the `notebook_login` method as follows:```
...
## 
	...
	](https://huggingface.co#3-register-the-meta-llama-31-405b-model-on-vertex-ai)
...
## 
	...
	](https://huggingface.co#4-deploy-meta-llama-31-405b-on-vertex-ai)
...
## 
	...
	](https://huggingface.co#5-run-online-predictions-with-meta-llama-31-405b)
...
### 
	...
	](https://huggingface.co#51-via-python)
...
#### 
	...
	](https://huggingface.co#511-within-the-same-session)
...
Prediction(predictions=["Yer want ta know about them fancy science things, eh? Alright then, matey, settle yerself down with a pint o' grog and listen close. I be tellin' ye about the Theory o' Relativity, as proposed by that swashbucklin' genius, Albert Einstein.\n\nNow, ye see, Einstein said that time and space be connected like the sea and the wind. Ye can't have one without the other, savvy? And he proposed that how ye see time and space depends on how fast ye be movin' and where ye be standin'. That be called relativity, me"], deployed_model_id='

/locations/', metadata=None, model_version_id='1', model_resource_name='projects/', explanations=None)/models/

#### 
	[
		
	](https://huggingface.co#512-from-a-different-session)
...
### 
	...
	](https://huggingface.co#52-via-the-vertex-ai-online-prediction-ui)
...
## 
	...
	](https://huggingface.co#6-clean-up-resources)
