---
title: 'Unpacking sandbox startup latency: why started is not ready'
topic: infra-platform
subtopic: deployment
secondary_topics:
- agents/computer-use
summary: Breaks down sandbox startup latency and why ready-state semantics matter
  for agent and remote-execution workflows.
source: modal
url: https://modal.com/blog/unpacking-sandbox-startup-latency
author: null
published: '2026-06-19'
fetched: '2026-07-11T04:26:52Z'
classifier: codex
taxonomy_rev: 1
words: 1500
content_sha256: 60f6fda001586423528e35a0b7f83284c468c98aa26a1b65dd21ef1d3e8c6b5b
triage: keep
skip_reason: null
---

# Unpacking sandbox startup latency: why started is not ready
...
It's tempting to think of a sandbox as "running" the moment its container is scheduled and booted. Many benchmarks, like [ComputeSDK's sandbox leaderboard](https://www.computesdk.com/benchmarks/sandboxes/), report on this as time to interactive (TTI). This is often measured from the moment you call `create()` to the first successful command running inside the container. It's useful for understanding the floor of what a provider can do, but it’s often just the start of the full startup chain.

Modal treats these pieces as [several distinct events](https://modal.com/docs/guide/sandboxes#events):

- **Created:**The sandbox has been requested but no compute resources are allocated yet. On Modal,- `Sandbox.create`is asynchronous and returns immediately. The latency addition here is negligible.
- **Scheduled:**The sandbox has been assigned to a worker, which is provisioning the resources it needs (CPU, memory, GPU, volumes) and preparing the container environment. The latency getting to this point depends on how fast your provider is able to quickly find the capacity you need.
- **Started:**The container is live, the entrypoint process is running, and network tunnels and Volume mounts are active. You can now run commands inside it with- `exec(...)`. The latency of this step depends on actual container boot time. This is usually what benchmarks measure.
- **Ready:**Your application-level initialization has finished and the sandbox can actually do the work your user wanted. The latency here is entirely use case specific, but is often multiple seconds.
- **In use:**The sandbox is- ~~cooking~~handling real work.

![](https://modal-cdn.com/cdnbot/sandbox lifecyclei50pc353_8b8e79d0.webp)
...
On Modal, a [Sandbox pool](https://modal.com/docs/examples/sandbox_pool) uses a `modal.Queue` to hold references to pre-warmed sandboxes. A background producer creates them, runs application setup, and pushes them in. When your app needs one, it pops one out and immediately spawns a replacement to keep the pool full.
...
![](https://modal-cdn.com/cdnbot/tmp2e5qm8bu_07a7a768.webp)
...
def fill_pool():
    ...
    sb.detach()
```
This also helps outside warm pools. Even when creating sandboxes on demand, `wait_until_ready()` gives you a clean sequencing primitive. You start the sandbox, wait for it to be useful, then proceed, rather than scattered timeouts throughout your startup logic.
...
![](https://modal-cdn.com/cdnbot/timetoreadyizwmbn0d_aa149133.webp)

## Get Started

Readiness probes are generally available today. Read [the docs](https://modal.com/docs/guide/sandboxes#readiness-probes) to learn more or start using them with `wait_until_ready()`and the new ready event will show up on your dashboard timeline. Check out our [warm pool example](https://modal.com/docs/examples/sandbox_pool) for a full implementation with TTL tracking, pool maintenance, and health checks.
