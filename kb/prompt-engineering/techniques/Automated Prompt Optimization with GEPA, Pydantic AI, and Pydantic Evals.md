---
title: Automated Prompt Optimization with GEPA, Pydantic AI, and Pydantic Evals
kind: blog
topic: prompt-engineering
subtopic: techniques
secondary_topics:
- evals-observability/evaluation
summary: Walks through automated prompt optimization with GEPA's evolutionary/reflective
  algorithm driven by Pydantic Evals as the scoring harness, using Agent.override()
  to inject candidate prompts without modifying agent definitions, turning manual
  prompt iteration into a systematic search of the prompt space against defined success
  criteria.
triage: null
skip_reason: null
source: pydantic
url: https://pydantic.dev/articles/prompt-optimization-with-gepa
author: David Montague
published: '2026-02-02'
fetched: '2026-07-16T22:04:35Z'
classifier: claude
taxonomy_rev: 2
words: 3141
content_sha256: 4c3feca5618c0d2866930243cf2cf24bc40adb1785924951cf05699c8a287829
---
...
- **Integration with**: We use- [Pydantic AI](https://pydantic.dev/pydantic-ai?utm_source=gepa_blogpost)- `Agent.override()`to inject candidate prompts during optimization without modifying agent definitions
- [Pydantic Evals](https://pydantic.dev/docs/ai/evals/evals/?utm_source=gepa_blogpost)provides the evaluation harness
...
- A dataset of test cases with expected outputs
- An evaluation function that scores how well the agent performs

Then let an algorithm systematically explore prompt variations to maximize that score.

This is exactly what [GEPA](https://github.com/gepa-ai/gepa?utm_source=gepa_blogpost) (Genetic-Pareto Prompt Evolution) does. In this article, we'll build a complete prompt optimization pipeline using GEPA with pydantic-ai and pydantic-evals, demonstrating how to turn prompt engineering from an art into a science.
...
- **Start with seed prompts**for each "module" (e.g., agent) in your system (your initial "pigeons")
- **Evaluate the system**against a dataset to get fitness scores
- **Generate variations**by having an LLM propose improvements based on what went wrong
- **Crossover**: Combine successful prompt variants from different modules to create new candidates
- **Keep the best performers**and discard the rest (Pareto selection)
...
- **Samples a mini-batch**from the training set
- **Evaluates the current best candidate**on that batch
- **Captures trajectories**—detailed information about what happened during execution
- **Builds a reflective dataset**—structured feedback about failures
- **Proposes mutations**—uses an LLM to suggest improvements based on the feedback
- **Accepts or rejects**the mutation based on whether it improves the score
...
- **Automation**: The process runs unattended—you can kick off an optimization and come back to improved prompts. Manual prompt iteration is attention-consuming; this frees you up.
- **Efficiency**: The genetic Pareto approach evaluates candidates on mini-batches and subsamples, letting you explore many more prompt variations than you could afford if you ran every candidate against the full dataset.
- **Fresh perspective**: The LLM proposer analyzes all failure cases together and suggests improvements without anchoring on your previous attempts. It might notice patterns you'd miss or try phrasings you wouldn't think of.
...
- Takes candidate prompts from GEPA
- Injects them into the agent using `Agent.override()`
- Runs evaluation using pydantic-evals
- Returns scores and feedback to GEPA
...
- Sample mini-batches and identify failures
- Generate improved prompts based on feedback
- Accept improvements that increase scores
- Repeat until convergence or budget exhaustion
...
- **Defines a structured task**with pydantic-ai
- **Creates a rich evaluation dataset**with pydantic-evals
- **Integrates with GEPA**through a custom adapter
...
The complete code is available in the [pydantic-stack-demo repository](https://github.com/pydantic/pydantic-stack-demo/tree/main/pai-gepa-prompt-optimization?utm_source=gepa_blogpost). To run it:

```
# Clone the repository
git clone https://github.com/pydantic/pydantic-stack-demo?utm_source=gepa_blogpost
