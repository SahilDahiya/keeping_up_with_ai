...
summary: 'Deep dive into Cloudflare''s CI-native AI code review built on OpenCode:
  up to seven specialized reviewer agents (security, performance, quality, docs, compliance)
  plus a coordinator that deduplicates findings and posts one structured review, run
  across tens of thousands of GitLab merge requests via a plugin architecture.'
source: cloudflare-ai
url: https://blog.cloudflare.com/ai-code-review/
author: Ryan Skidmore
published: '2026-04-20'
fetched: '2026-07-11T04:13:19Z'
classifier: claude
taxonomy_rev: 1
words: 5356
content_sha256: 25901cb13b405b516c747b9bf2ab829c5d79dbc85191110c515002267ad51dc1
---
...
Instead of building a monolithic code review agent from scratch, we decided to build a [ CI-native](https://developers.cloudflare.com/workers/ci-cd/) orchestration system around

[, an open-source coding agent. Today, when an engineer at Cloudflare opens a merge request, it gets an initial pass from a coordinated smörgåsbord of AI agents. Rather than relying on one model with a massive, generic prompt, we launch up to seven specialised reviewers covering security, performance, code quality, documentation, release management, and compliance with our internal Engineering Codex. These specialists are managed by a coordinator agent that deduplicates their findings, judges the actual severity of the issues, and posts a single structured review comment.](https://opencode.ai/)

__OpenCode__We've been running this system internally across tens of thousands of merge requests. It approves clean code, flags real bugs with impressive accuracy, and actively blocks merges when it finds genuine, serious problems or security vulnerabilities. This is just one of the many ways we’re improving our engineering resiliency as part of [ Code Orange: Fail Small](https://blog.cloudflare.com/fail-small-resilience-plan/).
...
- It has a great - __open source SDK__
...
`]` that may never arrive because the child process ran out of memory. In practice, it looks like this:
...
If a model provider goes down at 8 a.m. UTC when our colleagues in Europe are just waking up, we don’t want to wait for an on-call engineer to make a code change to switch out the models we’re using for the reviewer. Instead, the CI job fetches its model routing configuration from a [ Cloudflare Worker](https://workers.cloudflare.com/) backed by

[.](https://developers.cloudflare.com/kv/)

__Workers KV__The response contains per-reviewer model assignments and a providers block. When a provider is disabled, the plugin filters out all models from that provider before selecting the primary:
...
The system ships as a fully contained internal [ GitLab CI component](https://docs.gitlab.com/ci/components/). A team adds it to their
...
For more on how we’re using AI at Cloudflare, read our post on [ our internal AI engineering stack](https://blog.cloudflare.com/internal-ai-engineering-stack). And check out

[.](https://www.cloudflare.com/agents-week/updates/)

__everything we shipped during Agents Week__Have you integrated AI into your code review? We’d love to hear about it. Find us on __Discord__,[ X](https://x.com/cloudflaredev), and

[.](https://bsky.app/profile/cloudflare.social)

__Bluesky__Interested in building cutting edge projects like this, on cutting edge technology? __Come build with us!__
