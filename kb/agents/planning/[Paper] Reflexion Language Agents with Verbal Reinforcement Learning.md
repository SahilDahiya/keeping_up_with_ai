...
subtopic: planning
secondary_topics: []
summary: 'Agents improve by verbal self-reflection instead of weight updates: after
  a failed trajectory the agent writes a natural-language critique into episodic memory
  and retries, converting sparse reward into linguistic feedback. Delivers large gains
  on decision-making (ALFWorld), reasoning (HotpotQA), and coding (91% pass@1 on HumanEval)
  over ReAct/CoT baselines — a foundational self-improving agent loop.'
triage: null
skip_reason: null
source: arxiv
url: https://arxiv.org/abs/2303.11366
author: Noah Shinn, Federico Cassano, Edward Berman, Ashwin Gopinath, Karthik Narasimhan,
  Shunyu Yao
published: '2023-03-20'
fetched: '2026-08-01T19:06:57Z'
classifier: claude
taxonomy_rev: 2
words: 7433
content_sha256: 5e9ea459a660eaeadf1bbac6f0bebab9a8f84ae287e8c65c06d844e3b3a3755d
arxiv_id: '2303.11366'
categories:
- cs.AI
- cs.CL
...
**arXiv:** [2303.11366](https://arxiv.org/abs/2303.11366) · cs.AI, cs.CL, cs.LG
**Comment:** v4 contains a few additional experiments
...
We release all code, demos, and datasets at [https://github.com/noahshinn024/reflexion](https://github.com/noahshinn024/reflexion).

## 1 Introduction

Recent works such as ReAct Yao et al., ([2023](https://arxiv.org#bib.bib30)), SayCan Ahn et al., ([2022](https://arxiv.org#bib.bib1)),
Toolformer Schick et al., ([2023](https://arxiv.org#bib.bib22)), HuggingGPT (Shen et al.,, [2023](https://arxiv.org#bib.bib23)),
generative agents (Park et al.,, [2023](https://arxiv.org#bib.bib19)), and WebGPT (Nakano et al.,, [2021](https://arxiv.org#bib.bib17))
...
Reflexion agents improve on decision-making AlfWorld (Shridhar et al.,, [2021](https://arxiv.org#bib.bib24)) tasks over strong baseline approaches by
an absolute 22% in 12 iterative learning steps, and
on reasoning questions in HotPotQA (Yang et al.,, [2018](https://arxiv.org#bib.bib28)) by 20%, and Python programming
tasks on HumanEval (Chen et al.,, [2021](https://arxiv.org#bib.bib6)) by as much as 11%.
...
Self-Refine (Madaan et al.,, [2023](https://arxiv.org#bib.bib15)) employs an iterative framework for
...
but is limited to single-generation reasoning tasks. Pryzant et al., ([2023](https://arxiv.org#bib.bib21))
performs a similar semantic prompt-writing optimization, but is
also limited to single-generation tasks.
Paul et al., ([2023](https://arxiv.org#bib.bib20)) fine-tune critic models to provide intermediate
feedback within trajectories to improve reasoning responses.
Xie et al., ([2023](https://arxiv.org#bib.bib27)) use stochastic beam search over actions to
...
Yoran et al., ([2023](https://arxiv.org#bib.bib31)) and Nair et al., ([2023](https://arxiv.org#bib.bib16)) use decider models
to reason over several generations. Kim et al., ([2023](https://arxiv.org#bib.bib10)) use a
retry pattern over a fixed number of steps without an evaluation step.
Goodman, ([2023](https://arxiv.org#bib.bib9)) perform a qualitative evaluation step that
...
or code debugging practices. AlphaCode (Li et al.,, [2022](https://arxiv.org#bib.bib14))
evaluates a set of generations on hidden test cases. CodeT (Chen et al.,, [2022](https://arxiv.org#bib.bib5))
uses self-generated unit tests that are used to score generated function
implementations.
Self-Debugging (Chen et al.,, [2023](https://arxiv.org#bib.bib7)) employs a debugging component that is used
to improve existing implementations given feedback from a code
execution environment. CodeRL (Le et al.,, [2022](https://arxiv.org#bib.bib12)) sets the problem
...
| Self-refine (Madaan et al.,, [2023](https://arxiv.org#bib.bib15)) | ✓ | ✗ | ✗ | ✗ | ✗ | 
| Beam search (Xie et al.,, [2023](https://arxiv.org#bib.bib27)) | ✓ | ✓ | ✓ | ✓ | ✗ | 
| Reflexion (ours) | ✓ | ✓ | ✓ | ✓ | ✓ | 
...
| AlphaCode (Li et al.,, [2022](https://arxiv.org#bib.bib14)) | ✓ | ✗ | ✗ | ✓ | ✗ | 
| CodeT (Chen et al.,, [2022](https://arxiv.org#bib.bib5)) | ✓ | ✗ | ✓ | ✗ | ✗ | 
| Self-debugging (Chen et al.,, [2023](https://arxiv.org#bib.bib7)) | ✓ | ✓ | ✗ | ✗ | ✗ | 
| CodeRL (Le et al.,, [2022](https://arxiv.org#bib.bib12)) | ✓ | ✓ | ✗ | ✗ | ✗ | 
| Reflexion (ours) | ✓ | ✓ | ✓ | ✓ | ✓ | 

## 3 Reflexion: reinforcement via verbal reflection

We develop a modular formulation for Reflexion, utilizing three distinct models: an Actor, denoted as , which generates text and actions; an Evaluator model, represented by , that scores the outputs produced by ; and a Self-Reflection model, denoted as , which generates verbal reinforcement cues to assist the Actor in self-improvement. We provide a detailed description of each of these models and subsequently elucidate their collaborative functioning within the Reflexion framework.
...
or generation, , from the current policy  at time , receive
an observation from the environment . We explore various Actor models,
including Chain of Thought Wei et al., ([2022](https://arxiv.org#bib.bib26)) and ReAct Yao et al., ([2023](https://arxiv.org#bib.bib30)).
...
was inspired by Brooks et al., ([2022](https://arxiv.org#bib.bib3)), who suggest a policy iteration approach using in-context learning. Details on how this
...
action  led to subsequent incorrect actions  and . The
agent can then verbally state that it should have taken a different action,
, which would have resulted in  and , and store
this experience in its memory. In subsequent trials, the agent can leverage
its past experiences to adapt its decision-making approach at time  by
choosing action . This iterative process of trial, error, self-reflection,
...
Reflexion is formalized as an iterative optimization process in [1](https://arxiv.org#alg1).
In the first trial, the Actor produces a trajectory  by
interacting with the environment. The Evaluator then produces a score 
which is computed as .  is only a scalar
reward for trial  that improves as task-specific performance increases.
After the first trial, to amplify  to a feedback form that can be used
for improvement by an LLM, the Self-Reflection
model analyzes the set of  to produce a summary
 which is stored in the memory *mem*.  is a verbal
experience feedback for trial . The Actor, Evaluator, and Self-Reflection
models work together through trials in a loop until the Evaluator deems  to
be correct. As mentioned in [3](https://arxiv.org#S3.SS0.SSS0.Px4), the memory component of
Reflexion is crucial to its effectiveness. After each trial , , is appended
*mem*. In practice, we bound *mem* by a maximum number of stored experiences, 
...
search-based question answering on HotPotQA (Yang et al.,, [2018](https://arxiv.org#bib.bib28)), multi-step
tasks in common household environments in AlfWorld (Shridhar et al.,, [2021](https://arxiv.org#bib.bib24)), and code
writing tasks in competition-like environments with interpreters and
compilers in HumanEval (Chen et al.,, [2021](https://arxiv.org#bib.bib6)), MBPP (Austin et al.,, [2021](https://arxiv.org#bib.bib2)),
...
(Côté et al.,, [2019](https://arxiv.org#bib.bib8)). Following Yao et al., ([2023](https://arxiv.org#bib.bib30)), we run the agent in
...
chilling a tomato in the fridge). We use ReAct (Yao et al.,, [2023](https://arxiv.org#bib.bib30)) as the
action generator as Yao et al., ([2023](https://arxiv.org#bib.bib30)) has shown success in long trajectory
...
trajectory examples as Yao et al., ([2023](https://arxiv.org#bib.bib30)) with GPT-3 for the LLM. AlfWorld
...
In [3](https://arxiv.org#S4.F3), the learning curve suggests that
...
next 11 trials to a near-perfect performance. On the other hand, [3](https://arxiv.org#S4.F3)
...
HotPotQA (Yang et al.,, [2018](https://arxiv.org#bib.bib28)) is a Wikipedia-based dataset with 113k
...
+ Chain-of-Thought (CoT) (Wei et al.,, [2022](https://arxiv.org#bib.bib26)) for step-by-step  and
,  implementations, where  is the question, 
is the ground truth context from the dataset, and  is the final answer.
Since CoT is not a multi-step
decision-making technique, we give  to the agent so that we can isolate
...
implement a Reflexion + ReAct (Yao et al.,, [2023](https://arxiv.org#bib.bib30)) agent that can retrieve
...
the decision-making setup [4.1](https://arxiv.org#S4.SS1) in AlfWorld with a
...
step for reasoning using CoT (GT) as the baseline approach [4](https://arxiv.org#S4.F4).
Recall that CoT (GT)
...
explanation using language written in the first person. [4](https://arxiv.org#S4.F4)
...
writing on MBPP (Austin et al.,, [2021](https://arxiv.org#bib.bib2)), HumanEval (Chen et al.,, [2021](https://arxiv.org#bib.bib6)),
and LeetcodeHardGym, our new dataset. MBPP and HumanEval measure function body
generation accuracy given natural language
descriptions. We use a benchmark language compiler, MultiPL-E (Cassano et al.,, [2022](https://arxiv.org#bib.bib4)),
...
October 8, 2022, which is the pre-training cutoff date of GPT-4 (OpenAI,, [2023](https://arxiv.org#bib.bib18)).
...
(Wei et al.,, [2022](https://arxiv.org#bib.bib26))
...
sample  tests from the collection of generated unit tests to produce a
test suite , denoted as . We set  to a maximum
...
|---|---|---|---|
| HumanEval (PY) | 65.8 (CodeT (Chen et al.,, [2022](https://arxiv.org#bib.bib5)) + GPT-3.5) | 80.1 (GPT-4) | 91.0 | 
| HumanEval (RS) | – | 60.0 (GPT-4) | 68.0 | 
| MBPP (PY) | 67.7 (CodeT (Chen et al.,, [2022](https://arxiv.org#bib.bib5)) + Codex (Chen et al.,,[2021](https://arxiv.org#bib.bib6))) | 80.1 (GPT-4) | 77.1 | 
...
[2022](https://arxiv.org#bib.bib4)). TP: unit tests pass, solution pass; FN: unit tests fail, solution pass; FP: unit tests pass, solution fail; TN: unit tests fail, solution fail.

Reflexion outperforms all baseline accuracies and sets new state-of-the-art standards
on all benchmarks for Python and Rust except for MBPP Python [1](https://arxiv.org#S4.T1).
...
solution and lead to a false positive label on a code completion (Lam et al.,, [2020](https://arxiv.org#bib.bib11)).
...
report an invalid submission. In [2](https://arxiv.org#S4.T2), various conditions
...
Python. In [2](https://arxiv.org#S4.T2), we observe a notable discrepancy between
...
current implementations. [3](https://arxiv.org#S4.T3) shows an inferior
...
[2023](https://arxiv.org#bib.bib13)).
...
In [5](https://arxiv.org#S5), we briefly state that Reflexion struggles to overcome
local minima choices that require extremely creative behavior to escape. We observe this
shortcoming in an experiment on WebShop ([Yao et al., rint, ](https://arxiv.org#bib.bib29)). WebShop is a web-based
...
[6](https://arxiv.org#A2.F6). Further, the agent does not generate helpful,
