...
source: arize
url: https://arize.com/blog/how-to-build-an-ai-agent/
author: Sri Chavali
published: '2025-02-18'
fetched: '2026-07-11T04:51:29Z'
classifier: codex
taxonomy_rev: 1
words: 2863
content_sha256: a60c1203bec676e5a45621aadb66d29f4eef28231cd99fe68ed167bfa9cbdada
---

# How to Build An AI Agent

![How to Build an AI Agent-blog Text reads: how to build an ai agent with smolagents, AutoGen, and Langgraph above](https://arize.com/wp-content/uploads/2025/02/How-to-Build-an-AI-Agent-blog-1021x560.jpg)
...
hfModel = HfApiModel()
```
- Model_id is just a placeholder; replace it with a different model.
...
- add_base_tools=True: A parameter that adds some basic built-in tools (e.g., for text manipulation).
...
```
- Managed Agent: We transform our search_agent and code_worker_agent into ManagedAgents so we can keep better track of what they do (e.g., logging, chaining tasks, etc.).
- Manager Agent: This final “boss” agent is a CodeAgent that has both managed_agent_search and managed_agent_code under its watch.
...
```
When you call manager_agent.run(…), it can delegate parts of the request to the appropriate underlying agent(s). That’s it! With a ManagedAgent and a manager agent, you can keep your AI agents organized and ready for more complex workflows.
...
```
Config_list: tells agent which model to use and the associated API key. Most useful if you plan to switch between different model chaining.
...
- These agents are essentially your AI ‘bots,’ each configured using the model information in the config_list.
...
- The human_input_mode = “NEVER” indicated that there will not be interactive user prompts in real time.
...
feedback = user_proxy.initiate_chat(
   ...
   max_turns=1)
```
When you call user_proxy.initiate_chat(…), it can delegate parts of the request to the appropriate underlying agent. It’s where the actual ‘chat’ begins.

This example demonstrates a two-step process using two specialized agents—a writer and an editor. First, the user_proxy agent asks the writer_agent to compose a short story about a space explorer who learns a lesson on friendship. Next, it passes that story to the editor_agent for constructive feedback and suggestions. This showcases how multiple agents can collaborate to create and refine content in a single workflow.
