---
title: 'Doomed to Code: How we Teamed Up with Fireworks at MistralAI Hackathon to
  Conquer the Shores of Hell'
kind: blog
topic: agents
subtopic: computer-use
secondary_topics:
- models/multimodal
summary: Hackathon writeup on getting text-only Mistral-7B to play Doom by representing
  the game screen as text (bounding boxes of detected objects) rather than using vision
  models, working around the lack of native multimodal input to give a text LLM real-time
...
fetched: '2026-08-12T06:30:13Z'
classifier: claude
taxonomy_rev: 2
words: 2829
content_sha256: 18288449618f7e40ee2a6d926e62000c802b73295f551cde793a7973ba9e404a
---
...
Doom is a video game created in 1993 and has since gained cult status among hackers who have sought to run it on various hardware such as [calculators, ATMs, fridges, and even a toaster](https://www.youtube.com/watch?v=R6k72DI0Qo0&ab_channel=MashableDeals).

At the Mistral AI hackathon, it was obvious that we just **HAD** to see if Mistral-7B could play Doom.

Fig 1. Overview of our approach

We decided to use Fireworks for our hackathon project due its ease of use and developer experience. It enabled us to rapidly iterate during the short 24 hour timeline of the hackathon and quickly deploy our model minutes before the demo.
...
4. Performing evasive maneuvers when low on life or ammo
5. Avoiding harmful terrain e.g. acid or lava
6. Higher level strategies such as switching weapons or leveling up before engaging difficult enemies
...
- •***MOVE_FORWARD** : This action would cause the player to collide with the wall in front, so it has no utility in this situation.*
- •***MOVE_BACKWARD** : Moving backward could be useful to create some distance from the corner, allowing the player to reassess their position and potentially turn to face a more open area.*
- •***MOVE_LEFT** : Similar to moving forward, moving left would result in the player colliding with the wall to their left, so this action has no utility.*
- •***MOVE_RIGHT** : Moving right could be beneficial if there is open space to the player's right, as it would allow them to move away from the corner and potentially explore a new area.*
- •***TURN_LEFT** : Turning left would cause the player to face the wall directly, which is not particularly useful in this situation.*
- •***TURN_RIGHT** : Turning right could be the most useful action, as it would allow the player to face away from the corner and potentially move into an open area, depending on what lies to their right.*
- •***ATTACK** : Attacking has no utility in this scenario, as there is no mention of any enemies or objects to interact with.*
...
Many prior game playing agents such as [OpenAI Five](https://en.wikipedia.org/wiki/OpenAI_Five) and [DeepMind AlphaStar](<https://en.wikipedia.org/wiki/AlphaStar_(software)>) use reinforcement learning with self-play to train a policy network that produces actions at each timestep given visual input and/or language instructions.

These approaches use a policy gradient method e.g. [PPO](https://en.wikipedia.org/wiki/Proximal_policy_optimization), that trains the policy network to maximize total reward across episodes. In our case, the formulation would be very similar to the other games.
...
The next important part of our project was to generate a prompt to tell the model how to interpret the ASCII representation and generate actions. To do so, we imbued it with the identity `Demon Slayer`.

You're the DOOM AI, assuming the role of `Demon Slayer` in a grid environment represented by ASCII characters.
...
Fine-tuning the model was the final step before we could summon the `Demon Slayer`.
...
It was now 9 AM, the morning of the final day. We barely got any sleep and the model still hadn’t been trained. We decided to peruse the notion document shared by the hackathon organizers to see if we could leverage any other resources. That’s when we decided to try [fireworks.ai](https://fireworks.ai/).

We frantically added our [dataset](https://huggingface.co/datasets/CV-Mistral-Hackathon/doom-mistral-final) to Fireworks:
...
**T-2 hours:** Our model wasn’t working as expected. We had to figure out what was wrong - fast.
...
**T-1 hour**: After poring through the data, we finally discovered the bug with our prompt.

You are the DOOM AI, cast as the 'Demon Slayer' in a grid environment depicted using ASCII characters

***Your response should be straightforward, simply stating your action, like: `MOVE_FORWARD`**.*
...
**T-30 minutes**: With less than 30 minutes left till submission, we scoured the fireworks.ai documentation to figure out how to download the weights and run the model. Turns out that wasn’t necessary.

Within seconds the model was up and the `Demon Slayer` was now officially alive.
...
[Fig. 13 Marc Andreessen’s Tweet](https://www.linkedin.com/in/harshmdesai/)

[Fig. 14 Paige's tweet about our project](https://x.com/DynamicWebPaige/status/1771990295095419055)

Fireworks’ blazing fast inference stack and easy-to-use fine-tuning API helped us to iterate quickly. Integration with Weights and Biases helped us debug the model efficiently. The command line tool firectl made deploying the model so easy.

Lastly, [Fireworks fine-tuning service](https://readme.fireworks.ai/docs/fine-tuning-models), you can fine-tune and store up to 100 LoRA models readily available to be deployed for serverless inference.

Working with the [Fireworks.ai](https://fireworks.ai/) framework was seamless and played a pivotal role in making our project come to life.
