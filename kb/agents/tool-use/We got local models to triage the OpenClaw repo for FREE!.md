...
summary: Uses local Gemma/Qwen models inside an agent harness with structured outputs
  to triage hundreds of daily OpenClaw issues/PRs on a 128GB NVIDIA GB10, replacing
  a quota-limited GPT-5/Opus workflow; covers label schema design, prompt iteration
  and accuracy vs closed models.
triage: null
skip_reason: null
source: huggingface
...
fetched: '2026-07-14T22:04:50Z'
classifier: claude
taxonomy_rev: 1
words: 2785
content_sha256: e74632dc13f2860f511eebb571d5f600542c04d5cf868543a61d1b652c5b2a1e
---
...
# 
	...
	](https://huggingface.co#we-got-local-models-to-triage-the-openclaw-repo-for-free)
	
		We got local models to triage the OpenClaw repo for FREE!*
	

 [Update on GitHub](https://github.com/huggingface/blog/blob/main/local-models-pr-triage.md)

[  Upvote 9 ](https://huggingface.co/login?next=%2Fblog%2Flocal-models-pr-triage)

![Onur Solmaz's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/633413b18003819e16aee0aa/CglgixDB95u8ILW8WdxCU.png) 
...
## 
	...
	](https://huggingface.co#categorizing-issues-and-prs)
	
		Categorizing issues and PRs
	

We came up with a finite set of labels representing the categories of issues we need to triage, and then use a local model to classify each issue into one of those categories, like `local_models`, `self_hosted_inference`, `acp`, `agent_runtime`, `codex`, `ui_tui` and so on.[[3]](https://huggingface.co#note-3)
...
For that reason, we use [ reposhell](https://github.com/osolmaz/localpager/tree/main/reposhell) instead of 

`bash`: a restricted `bash`-like shell that only allows read-only operations (`ls`, `find`, `cat`, `grep`, etc.) on the OpenClaw repo. The model thinks it is using `bash`, but any operation that is not allowed is rejected:```
...
## 
	...
	](https://huggingface.co#processing-incoming-prs-and-issues)
...
- We use [openclaw/gitcrawl](http://github.com/openclaw/gitcrawl)to act as a local mirror for the repo. Whenever there is a new PR or issue, each item is normalized into the same shape and written into localpager's own SQLite database. If the item is new, localpager creates a classification job for it.
...
## 
	...
	](https://huggingface.co#can-local-models-triage-prs)
...
## 
	...
	](https://huggingface.co#tracking-and-validating-real-time-performance-using-openclaw)
	
		Tracking and validating real-time performance using OpenClaw
	

We have mentioned earlier that instead of running a job with a local model for every new issue or PR, we can run a batch job with a SOTA cloud model, like GPT-5.5 running in OpenClaw, every n hours (e.g. every 2 hours) to achieve the same end.[[5]](https://huggingface.co#note-5)
...
- For the use case in this post, we have discovered that breaking down a PR/Issue in a way that means the product surface is understood and labelled correctly is a hard problem. [back](https://huggingface.co#note-1-ref)
- Although in our testing we didn't---it would be quite reasonable for a model to conclude a next-step to gather info, use an external classifier. The agentic approach and the traditional approach are not mutually exclusive. [back](https://huggingface.co#note-2-ref)
- See full list of topics and other configuration [here](https://github.com/osolmaz/localpager/blob/main/examples/profiles/openclaw-routing-topics.json).[back](https://huggingface.co#note-3-ref)
- We used `DeepSeek-V4-Flash-IQ2XXS-w2Q2K-AProjQ8-SExpQ8-OutQ8-chat-v2.gguf`from[antirez/deepseek-v4-gguf](https://huggingface.co/antirez/deepseek-v4-gguf).[back](https://huggingface.co#note-4-ref)
- While we are aware that using an LLM as a judge negates the "free" aspect, our specific implementation does this for research purposes. In practice, a bigger and more expensive model can be used in tandem during a trial period for calibration, after which the system would transition fully to the smaller one. In recent runs, this audit loop used roughly 40k total GPT-5.5 tokens per 2-hour check, mostly cached context, costing about 2-3 cents per run at API pricing, or roughly $9/month at 12 runs per day. This was a single batch audit across all new items, not one judge call per item; doing it per item would likely be several times more expensive. [back](https://huggingface.co#note-5-ref)
