...
summary: Fireworks' OpenAI-compatible Responses API adds first-class Model Context
  Protocol (MCP) support, moving the agentic loop (reasoning, tool selection, execution)
  server-side so any MCP-speaking tool integration is portable across models rather
  than locked into one vendor's tool-calling format.
triage: null
skip_reason: null
source: fireworks
