...
summary: Engineering writeup on building the Langfuse MCP server, focusing on code
  reuse, developer experience, and exposing observability workflows to agents.
source: langfuse
url: https://langfuse.com/blog/2025-12-09-building-langfuse-mcp-server
author: null
published: '2025-12-09'
fetched: '2026-07-11T04:36:02Z'
classifier: codex
taxonomy_rev: 1
words: 1693
content_sha256: 5ab4d09f61360f90e967f9811a4c0c6399286371a2e1f1116fff8e0f1cb0ec9a
---
...
How we built a production MCP server by reusing existing API logic, creating better abstractions, and learning what matters for LLM tool usage.

![Picture Michael Froehlich](https://langfuse.com/_next/image?url=%2Fimages%2Fpeople%2Fmichaelfroehlich.jpg&w=96&q=75) Michael Froehlich

Michael FroehlichThe Model Context Protocol (MCP) enables AI assistants to interact directly with external tools and data sources. We built an MCP server for Langfuse to let developers manage prompts from their AI coding assistants without context switching.

This post covers the technical decisions and implementation details of our MCP server.

[Scoping: Starting with Prompt Management](https://langfuse.com#scoping-starting-with-prompt-management)
...
The current tool list, input schemas, and setup snippets are available in the generated [MCP Reference](https://mcp.reference.langfuse.com).
...
[Architecture: Same Logic, Different Entry Point](https://langfuse.com#architecture-same-logic-different-entry-point)
...
```
*( View the actual implementation on GitHub)*
...
[Making Tools Easy: The defineTool Helper](https://langfuse.com#making-tools-easy-the-definetool-helper)
...
```
*( View defineTool implementation on GitHub)*
...
- **Tool definition**: JSON Schema for the MCP protocol
- **Wrapped handler**: Validates inputs, catches errors, formats responses

Adding new tools is straightforward: define schemas, write handler, register.

[Tool Descriptions: Writing for LLMs](https://langfuse.com#tool-descriptions-writing-for-llms)
...
[Authentication: Choosing BasicAuth](https://langfuse.com#authentication-choosing-basicauth)
...
[The Details That Matter](https://langfuse.com#the-details-that-matter)

Implementing the MCP server, we had to deal with a few technical implementation details.

[Stateless Per-Request Servers](https://langfuse.com#stateless-per-request-servers)
...
[Error Handling: User vs Server Errors](https://langfuse.com#error-handling-user-vs-server-errors)
...
```
*( View error handling on GitHub)*

[Feature Modules: Self-Contained and Extensible](https://langfuse.com#feature-modules-self-contained-and-extensible)
...
```
*( View the registry implementation on GitHub)*
...
[Monitoring: The SSE Gotcha](https://langfuse.com#monitoring-the-sse-gotcha)
...
[Key Takeaways](https://langfuse.com#key-takeaways)
