...
secondary_topics:
- agents/planning
summary: Covers systematic evaluation of AI agents, focusing on experiment interpretation,
  failure analysis, and how to compare agent variants.
source: langfuse
url: https://langfuse.com/blog/2025-11-06-experiment-interpretation
author: null
published: '2025-11-06'
fetched: '2026-07-11T04:35:52Z'
classifier: codex
taxonomy_rev: 1
words: 1329
content_sha256: 5de10b6705be513be031ef698040400467ba8df123ad78cba2eb4db801590cc7
---
...
We'll demonstrate running experiments via the Langfuse SDK. For other experiment execution methods please refer to [docs](https://langfuse.com/docs/evaluation/core-concepts#experiments).

[Run experiment](https://langfuse.com#run-experiment)

Running the experiment involves three components: the **task**, the **data**, and the **evaluators**.

[1. Task](https://langfuse.com#1-task)
...
[2. Data](https://langfuse.com#2-data)
...
[3. Evaluators](https://langfuse.com#3-evaluators)
...
[Managed Execution](https://langfuse.com#managed-execution)
...
print(result.format())
```
[Interpret experiments results](https://langfuse.com#interpret-experiments-results)

As we have successfully run our experiment, interpretation follows a top-down funnel:

[1. The Macro View (All Experiments)](https://langfuse.com#1-the-macro-view-all-experiments)
...
![Experiments table](https://langfuse.com/_next/image?url=%2Fimages%2Fblog%2F2025-11-06-experiment-interpretation%2Fexperiments-table.png&w=3840&q=75)


[2. The Baseline Comparison (The "Diff")](https://langfuse.com#2-the-baseline-comparison-the-diff)
...
[Step 1: Review Aggregate Metrics](https://langfuse.com#step-1-review-aggregate-metrics)
...
![Experiment charts](https://langfuse.com/_next/image?url=%2Fimages%2Fblog%2F2025-11-06-experiment-interpretation%2Fcompare-charts.png&w=3840&q=75)


[Step 2: Isolate Regressions and perform Root Cause Analysis](https://langfuse.com#step-2-isolate-regressions-and-perform-root-cause-analysis)
...
- **Filtering:**This is the primary tool. Use column header summaries to filter for specific value failures (e.g.,- `Candidate Correctness < 0.3`).
- **[Coming soon] Sorting:**Sort the table by the scores to surface the single worst-performing items immediately.
...
[3. Exploring Results with Human Annotation](https://langfuse.com#3-exploring-results-with-human-annotation)
...
Our goal is to convert expert human judgment into a structured, queryable `score`. Given our example, this might involve a two-stage annotation workflow. Note that annotation workflows strongly differ by use case. For more detail please see our Academy page on [Error Analysis](https://langfuse.com/academy/monitoring/error-analysis).
...
- **Run:**Execute the- `Task`against- `Data`and assess resulting outputs using- `Evaluators`.
- **Analyze:**Compare the candidate run to a baseline in a top-down funnel (Aggregate Metrics → Item-level Diff → Trace-level Debugging).
