...
summary: Design deep dive on the runtime for Langfuse code evaluators, covering execution
  isolation, evaluator lifecycle, and safe scalable scoring infrastructure.
source: langfuse
url: https://langfuse.com/blog/2026-06-22-code-evaluators-execution-model
author: null
published: '2026-06-22'
fetched: '2026-07-11T04:36:40Z'
classifier: codex
taxonomy_rev: 1
words: 2242
content_sha256: 8374837d80dc5ec8adedb2720702633282b1e480cdc44ad44c361f0d288219e1
---
...
![Picture Tobias Wochinger](https://langfuse.com/_next/image?url=%2Fimages%2Fpeople%2Ftobiaswochinger.jpg&w=96&q=75) Tobias Wochinger

Tobias WochingerIn late May we shipped [code evaluators](https://langfuse.com/docs/evaluation/evaluation-methods/code-evaluators): you write a small Python or TypeScript function and Langfuse runs it to score your traces. By design, this means effectively anybody can run untrusted code in our multi-tenant SaaS environment: an environment that holds petabytes of critical data from thousands of teams.
...
[Why code evaluators](https://langfuse.com#why-code-evaluators)

Evaluation is how you find out whether your LLM app is actually any good. [Reading traces yourself](https://langfuse.com/blog/2026-06-09-ai-is-eating-ai-engineering) is where your opinion of your application gets formed, and nothing replaces it. But nobody reads millions of traces by hand. [Error analysis](https://langfuse.com/academy/monitoring/error-analysis) turns that reading into a list of error modes, and the standard move is to encode each one as an [LLM-as-a-judge evaluator](https://langfuse.com/docs/evaluation/evaluation-methods/llm-as-a-judge) that scores every trace for things like agent helpfulness, conciseness, or tone.
...
```
[Small workloads with hard constraints](https://langfuse.com#small-workloads-with-hard-constraints)
...
[Options we considered](https://langfuse.com#options-we-considered)
...
[Our runtime of choice: AWS Lambda](https://langfuse.com#our-runtime-of-choice-aws-lambda)
...
The local dispatcher executes evaluator code in-process, without the isolation guarantees described in this post. It is not safe against cross-tenant access and must not be used in production / untrusted environments.

[Tenant isolation and warm environments](https://langfuse.com#tenant-isolation-and-warm-environments)
...
[The dispatch path: synchronous and inline](https://langfuse.com#the-dispatch-path-synchronous-and-inline)
...
[The principle of least privilege](https://langfuse.com#the-principle-of-least-privilege)
...
[Plan meets production](https://langfuse.com#plan-meets-production)
...
external IP
address](https://langfuse.com/_next/image?url=%2Fimages%2Fblog%2F2026-06-22-code-evaluators-execution-model%2Fguardduty-external-ip.png&w=3840&q=75)
...
extracted
credentials](https://langfuse.com/_next/image?url=%2Fimages%2Fblog%2F2026-06-22-code-evaluators-execution-model%2Fguardduty-access-denied.png&w=3840&q=75)
...
If you want to try code evaluators, the [docs](https://langfuse.com/docs/evaluation/evaluation-methods/code-evaluators) are the place to start. If one of the decisions outlined here blocks you, tell us in [GitHub Discussions](https://github.com/orgs/langfuse/discussions): what we add next is driven by what you run into.

And if running untrusted code across thousands of tenants sounds like a fun problem, [we are hiring](https://langfuse.com/careers).
